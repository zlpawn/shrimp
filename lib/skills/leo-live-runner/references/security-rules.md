...
动态执行引擎在编译前内置了细粒度、单一职责的 **七大安全规则族（SecurityRuleType，参考 Alibaba Druid WallFilter 标准）**。
AI 在生成动态代码时，必须严格遵守以下安全红线，否则会被引擎在编译前秒级阻断（Fail-Fast）。

---

## 1. 🔬 AST 语法树级沙箱与黑名单规约 (`AST_SANDBOX_SECURITY` / `AstSandboxSecurityRule`)
* **底层 AST 编译拦截**：利用 Groovy `SecureASTCustomizer` 在语法树解析阶段进行底层编译硬约束；
* **高危导入黑名单**：严禁 `import` 或通配符引入 `java.lang.System`、`java.lang.Runtime`、`java.lang.ProcessBuilder`、`sun.misc.*`、`jdk.internal.*`、`java.lang.reflect.*`（部分底层逃逸类）；
* **间接别名导入检查**：开启 `indirectImportCheck`，阻断通过 `as` 别名或间接转译逃逸沙箱。

---

## 2. 🗄️ SQL DML 安全规则 (`SQL_DML_SAFETY` / `SqlSafetyRule`)
...
## 3. 🧱 SQL DDL 严禁规约 (`SQL_DDL_SAFETY` / `SqlDdlSafetyRule`)
...
## 4. 🔴 Redis 高危操作禁令 (`REDIS_SAFETY` / `RedisSafetyRule`)
...
## 5. 🌱 Spring 容器与配置保护 (`SPRING_CONFIG_SECURITY` / `SpringConfigSecurityRule`)
...
## 6. 💻 系统与 JVM 进程安全 (`SYSTEM_SECURITY` / `SystemSecurityRule`)
...
## 7. 🧵 线程与死锁安全 (`THREAD_SECURITY` / `ThreadSecurityRule`)
