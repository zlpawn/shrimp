...
export async function findOrCreateTab(domainMatch, targetUrl) {
  ...
  let existing = null;
  if (domainMatch === "1hai.cn") {
    // 优先复用一嗨下单页 booking.1hai.cn
    existing = tabs.find((t) => t.url && t.url.includes("booking.1hai.cn")) || tabs.find((t) => t.url && t.url.includes("1hai.cn"));
  } else {
    existing = tabs.find((t) => t.url && t.url.includes(domainMatch));
  }

  if (existing) {
    return { id: existing.id, reused: true, tab: existing };
  }
  const newTab = await runCmd("tabs.new", { url: targetUrl, focus: true });
...
export async function withScraperTask(taskTitle, taskFn, options = {}) {
  const { sameWindow = true, focus = true } = options;
