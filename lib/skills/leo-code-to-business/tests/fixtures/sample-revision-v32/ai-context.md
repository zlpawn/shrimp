...
- Revision checkpoint：`passed`
- Whole-project status：`complete`
- 已完成模块：整改工单创建
...
- 实现概览：创建入口接收整改请求，服务层补齐人员与固定工单类型，随后由客户端调用外部工单系统；当前链路未发现本地事务或自动重试。
- `submit`：创建整改工单入口（WorkOrderController.create）
- `enrich`：整改工单请求补齐（WorkOrderService.create）
- `call-external`：外部工单创建客户端（WorkOrderController.create -> external work-order client）
- 改动指南：调整外部整改工单的类型选择规则；验证：默认输入仍生成 HOME2/TODO、新条件选择预期类型、外部空结果和失败仍返回失败、重复提交行为被明确验证、错误日志包含可定位信息
...
- Canonical revision：`2928ec7358dba11ccab8d9214d8698657a0cfa27e7321fb0054625a02e86507c`
