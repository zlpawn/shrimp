...
import { APOLLO_SERVERS, loadServiceCatalog, resolveAppId as resolveAppIdFromCatalog } from './common/services.js';
...
async function resolveAppId(inputApp, server, cluster) {
  ...
  // 1. 统一服务注册表与内置别名解析
  const canonical = resolveAppIdFromCatalog(inputApp);
  if (canonical && canonical.toLowerCase() !== lowerInput) {
    return { resolvedAppId: canonical, original: inputApp, reason: 'catalog_alias' };
  }
...
async function main() {
  ...
  if (positionalArgs.length === 2) {
    ...
    const isNamespaceExt = /\.(properties|json|xml|yml|yaml|txt)$/i.test(secondArg);
    if (isNamespaceExt || secondArg.toLowerCase() === 'application' || secondArg.toLowerCase() === 'default') {
