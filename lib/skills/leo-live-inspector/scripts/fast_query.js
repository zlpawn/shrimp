 ...
 *   4. 📝 三级通道（人机兜底）：探针均不可用时，优雅提示手动导入 Chrome 扩展或提供集群/索引。
 */
...
// 自动环境路由：若指定 --env test / -e test，自动委派给 test_log_query.js (Paoding Loki 容器日志引擎)
if (['test', 'dev', 'qa', 'local'].includes(opts.env)) {
  const argv = process.argv.slice(2);
  const testScript = path.join(__dirname, 'test_log_query.js');
  const filteredArgs = [];
  for (let i = 0; i < argv.length; i++) {
...
let matchedConfig = serviceCatalog[appKeyWithEnv] || serviceCatalog[opts.app] || serviceCatalog[resolveAppId(opts.app)] || serviceCatalog[opts.app.replace(/[:_\-](prod|test|dev)$/, '')];
    ...
    if (learned && learned.index) {
      ...
      try {
        ...
        console.error(JSON.stringify({
          ...
          suggestions: [
            `1. 可在 chrome://extensions/ 开启开发者模式，选择【加载已解压的扩展程序】，导入 ${path.join(__dirname, '..', 'resources', 'chrome_extension')} 目录`,
            "2. 或直接通过插件/F12 复制 Cookie 贴给 AI",
