...
const COOKIE_FILE = path.join(SHRIMP_DIR, 'test_apollo_cookie.json');
const DEFAULT_PORTAL_HOST = 'test-apollo.portal.life.ke.com';
const DEFAULT_CONFIG_SERVER = 'http://test.config.apollo.ke.com';
...
function loadCookie() {
  if (process.env.APOLLO_TEST_COOKIE) {
    return process.env.APOLLO_TEST_COOKIE.trim();
  }
  ...
  // 2. 检测 Downloads 下的 cookies.txt
  const candidates = [
    path.join(process.cwd(), 'cookies-test-apollo.portal.life.ke.com.txt'),
    path.join(process.cwd(), 'cookies.txt'),
    path.join(os.homedir(), 'Downloads', 'cookies-test-apollo.portal.life.ke.com.txt'),
    ...
    if (fs.existsSync(c)) {
      try {
        ...
        for (const line of lines) {
          ...
          if (parts.length >= 7 && (parts[0].includes('test-apollo') || parts[0].includes('.ke.com'))) {
...
function requestHttp(options, postBody = null) {
  return new Promise((resolve) => {
    const bodyStr = postBody ? (typeof postBody === 'string' ? postBody : JSON.stringify(postBody)) : null;
    ...
    req.on('timeout', () => {
      req.destroy();
      resolve({ statusCode: 0, error: 'Request timeout (10000ms)' });
    });
...
async function main() {
  ...
  if (setCookieIdx !== -1 && argv[setCookieIdx + 1]) {
    saveCookie(argv[setCookieIdx + 1]);
  ...
  const canonicalApp = ALIAS_MAP[inputApp.toLowerCase()] || inputApp;
