...
const COOKIE_FILE = path.join(SHRIMP_DIR, 'paoding_cookie.json');
...
const ALIAS_MAP = {
  ...
  'zulin-iot-platform': 'zulin-iot-platform',
...
function loadCookie() {
  if (process.env.PAODING_COOKIE) {
    return process.env.PAODING_COOKIE.trim();
  }
  ...
  // 2. 尝试从 Downloads/cookies.txt 或工程目录下提取
  const candidates = [
    path.join(process.cwd(), 'cookies-paoding.ke.com.txt'),
    path.join(process.cwd(), 'cookies.txt'),
    path.join(os.homedir(), 'Downloads', 'cookies-paoding.ke.com.txt'),
    ...
    if (fs.existsSync(c)) {
      try {
        ...
        for (const line of lines) {
          ...
          if (parts.length >= 7 && (parts[0].includes('paoding') || parts[0].includes('.ke.com'))) {
            kvs.push(`${parts[5].trim()}=${parts[6].trim()}`);
          }
        }
        if (kvs.length > 0) {
  ...
  // 3. macOS 下尝试调用 ego-browser 自动提取
  if (process.platform === 'darwin') {
    try {
      const script = `
        const spaces = await listTaskSpaces();
        const space = spaces[0];
        await claimTaskSpace(space.id);
        const cookiesObj = await cdp('Network.getCookies', { urls: ['https://paoding.ke.com'] });
        process.stdout.write(cookiesObj.cookies.map(c => \`\${c.name}=\${c.value}\`).join('; '));
      `;
      const out = execSync(`ego-browser nodejs -e "${script.replace(/\n+/g, ' ')}"`, {
        encoding: 'utf8',
        timeout: 4000,
        stdio: ['ignore', 'pipe', 'ignore']
      }).trim();
      if (out && out.includes('security_ticket')) {
        saveCookie(out);
        return out;
      }
    } catch {}
...
function saveCookie(cookieStr) {
  try {
    if (!fs.existsSync(SHRIMP_DIR)) fs.mkdirSync(SHRIMP_DIR, { recursive: true });
...
async function main() {
  ...
  if (setCookieIdx !== -1 && args[setCookieIdx + 1]) {
    saveCookie(args[setCookieIdx + 1]);
  ...
  const canonicalApp = ALIAS_MAP[appArg.toLowerCase()] || appArg;
  if (!size) {
    size = traceId ? 50 : 20;
  }
  ...
  const cookie = customCookie || loadCookie();
  if (!cookie) {
