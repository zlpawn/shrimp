...
function hasMultipleTopLevelStatements(tokens) {
  for (let index = 0; index < tokens.length; index += 1) {
    if (!tokenAt(tokens, index, 'punctuation', ';') || tokens[index].depth !== 0) continue;
    for (let following = index + 1; following < tokens.length; following += 1) {
      if (tokens[following].depth === 0 && !tokenAt(tokens, following, 'punctuation', ';')) return true;
    }
  }
  return false;
}
...
export function analyzeSql(sql) {
  ...
  const hasTopLevelWhere = guardRequired
    ? (complete && !hasMultipleTopLevelStatements(tokens) ? findTopLevelWhere(tokens, statementIndex) : null)
    : null;
