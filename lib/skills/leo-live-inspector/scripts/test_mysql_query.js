...
/**
 * 🗄️ Leo Live Inspector - 线下与测试环境 MySQL 直连查询引擎 (Test MySQL Direct Query Engine)
 * 
 * 核心特性：
...
function printUsage() {
  console.log(`
🔍 Leo Test MySQL Direct Query (线下/测试环境 MySQL 直连查询引擎)
...
参数说明:
  ...
  [sql]             待执行的 SQL 语句 (以 SELECT / SHOW / DESC / EXPLAIN 开头)
...
async function main() {
  ...
  if (positional.length === 1) {
    ...
    if (firstPos.startsWith('SELECT') || firstPos.startsWith('SHOW') || firstPos.startsWith('DESC') || firstPos.startsWith('EXPLAIN')) {
      sql = positional[0];
    } else {
      dsArg = positional[0];
      sql = positional[1];
  ...
  if (!outputJson) {
    console.log(`================================================================`);
    console.log(`🚀 Leo 测试环境 MySQL 直连查询: ${canonicalService} [${dsAlias}]`);
