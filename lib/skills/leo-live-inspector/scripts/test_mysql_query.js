...
async function main() {
  ...
  if (positional.length === 1) {
    ...
    const firstPos = positional[0].trim().toUpperCase();
    const SQL_KEYWORDS = ['SELECT', 'SHOW', 'DESC', 'EXPLAIN', 'INSERT', 'UPDATE', 'DELETE', 'REPLACE', 'CREATE', 'ALTER', 'DROP', 'TRUNCATE'];
    if (SQL_KEYWORDS.some(kw => firstPos.startsWith(kw))) {
      sql = positional[0];
    } else {
      dsArg = positional[0];
      sql = positional[1];
  ...
  const isDML = /^(INSERT|UPDATE|DELETE|REPLACE)\b/i.test(sqlNorm);
  const isDDL = /^(CREATE|ALTER|DROP|TRUNCATE)\b/i.test(sqlNorm);
  const isWrite = isDML || isDDL;

  // 8. 安全拦截: UPDATE/DELETE 无 WHERE 条件时阻断
  if (!forceMode && (sqlNorm.startsWith('UPDATE') || sqlNorm.startsWith('DELETE'))) {
    // 简易检测: 去掉子查询和字符串常量后检查是否包含 WHERE
    const stripped = sqlNorm.replace(/'[^']*'/g, '').replace(/"[^"]*"/g, '').replace(/\([^)]*\)/g, '');
    if (!stripped.includes('WHERE')) {
      console.error(`\n🛑 安全拦截: 检测到无 WHERE 条件的 ${sqlNorm.startsWith('UPDATE') ? 'UPDATE' : 'DELETE'} 操作！`);
      console.error(`   这将影响整张表的所有数据行，已自动阻断。\n`);
  ...
  try {
    ...
    if (isWrite && rows && typeof rows.affectedRows === 'number') {
      ...
      if (isDML && sqlNorm.startsWith('INSERT')) {
        console.log(`| 自增ID (insertId) | ${rows.insertId || 0} |`);
      }
      if (isDML && (sqlNorm.startsWith('UPDATE') || sqlNorm.startsWith('REPLACE'))) {
