(() => {
  ...
  function cancelTuck() {
    ...
    ball.classList.remove("is-tucked");
  }

  function scheduleTuck() {
    cancelTuck();
    if (panelOpen || dragging || rotating) return;
    tuckTimer = setTimeout(() => {
      if (panelOpen || dragging || rotating) return;
      ball.classList.add("is-tucked");
    }, 2000);
  }
  ...
  function applyTransform() {
    ...
    const isLeft = state.x + SIZE / 2 < window.innerWidth / 2;
    ball.classList.toggle("is-docked-left", isLeft);
    ball.classList.toggle("is-docked-right", !isLeft);
  ...
  function endGesture(event) {
    ...
    if (wasDragging && moved) {
      ...
      const snapLeft = state.x + SIZE / 2 < window.innerWidth / 2;
      state.x = snapLeft ? MARGIN : window.innerWidth - SIZE - MARGIN;
      applyTransform();
    }
