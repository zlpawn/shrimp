...
function printHelp() {
  console.log(`
Leo Trend Intelligence CLI Helper (leo_trend_intel.mjs)

用法 (Usage):
  node leo_trend_intel.mjs [options]
...
async function main() {
  ...
  if (options.action === "events") {
    try {
      ...
      const events = res?.data?.events || (Array.isArray(res?.events) ? res.events : (Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : [])));
  ...
  try {
    ...
    const brief = (res && res.data && (res.data.markdown || res.data.date)) ? res.data : res;
    if (options.format === "json") {
