 ...
 *
 * `scheduler.enabled` in trend-intel config acts as the master switch for the whole
 * scheduler process, while `crawl_enabled` / `brief_enabled` allow disabling only
 * one of the two sub-tasks (they are managed by the Job Scheduler center so each
 * job card can be toggled independently without killing the shared minute timer).
 *
...
export function createTrendIntelScheduler(service, options = {}) {
  ...
  function crawlEnabled(config = {}) {
    return config.crawl_enabled !== false;
  }

  function briefEnabled(config = {}) {
    return config.brief_enabled !== false;
  }

  async function runCrawl() {
    try {
      const config = service.getConfig()?.scheduler || {};
      if (config.enabled === false || !crawlEnabled(config)) return;

      const result = await service.crawlOnce({ isScheduled: true });
  ...
  async function checkDailyBrief() {
    try {
      const config = service.getConfig()?.scheduler || {};
      if (config.enabled === false || !briefEnabled(config)) return;
  ...
  function start() {
    ...
    // Trigger immediate initial crawl in background (0-token network fetch)
    if (options.initialCrawl !== false) {
      setImmediate(() => {
        runCrawl().catch(err => {
          if (typeof logger.error === "function") {
            logger.error(`[TrendIntelScheduler] Initial crawl error:`, err?.message || err);
          }
        });
      });
    }
  ...
  /**
   * Re-arm the crawl timer with the latest interval from service config,
   * without re-triggering the initial crawl (unlike a full stop/start).
   * Safe to call whether the scheduler is running or stopped.
   */
  function rescheduleCrawl() {
    if (crawlTimer) {
      clearInterval(crawlTimer);
      crawlTimer = null;
    }
    ...
    const intervalMin = Math.max(1, Number(options.intervalMinutes) || Number(config.interval_minutes) || 30);
    crawlTimer = setInterval(runCrawl, intervalMin * 60 * 1000);
    if (typeof logger.info === "function") {
      logger.info(`[TrendIntelScheduler] Crawl timer re-armed with interval: ${intervalMin}m`);
    }
  }
  ...
  function getStatus() {
    ...
    return {
      ...
      crawl_enabled: crawlEnabled(config),
      brief_enabled: briefEnabled(config),
