...
/**
 * Strip HTML tags and normalize whitespace for clean plain text rendering.
 * @param {string} html
 * @returns {string}
 */
 ...
 */
export function parseRssXml(sourceId, xmlText, feedUrl = "", extraMeta = {}) {
  ...
  for (let i = 0; i < itemMatches.length; i++) {
    ...
    items.push({
      ...
      raw: {
        ...
        url: link,
        description,
        summary: stripHtmlToPlainText(description).slice(0, 500),
        platformName: extraMeta.platformName || extraMeta.name || normalizedSource,
        icon: extraMeta.icon || "📰",
        pubDate: pubDateStr,
        guid,
        author,
...
export async function fetchRssFeed(url, options = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      return parseRssXml(sourceId || url, xmlText, url, options);
    } catch (err) {
      lastError = err;
