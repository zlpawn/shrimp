  ...
  geek_feeds: [
    {
      ...
      name: "LINUX DO 每日热榜",
      icon: "🐧",
      url: "https://linux.do/top/daily.rss",
      enabled: true,
      category: "community"
    },
    {
...
export function createTrendIntelConfigStore(dataDir) {
  ...
  function get() {
    ...
    try {
      ...
      return {
        ...
        geek_feeds: Array.isArray(parsed?.geek_feeds) ? parsed.geek_feeds : DEFAULT_CONFIG.geek_feeds
      };
    } catch {
      return JSON.parse(JSON.stringify(DEFAULT_CONFIG));
    }
  ...
  function update(patch) {
    ...
    const next = {
      ...
      geek_feeds: Array.isArray(patch.geek_feeds) ? patch.geek_feeds : current.geek_feeds
    };
