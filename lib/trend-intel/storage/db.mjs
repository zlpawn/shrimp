...
export function createTrendIntelDb(dataDir) {
  ...
  function getRawItems(options = {}) {
    ...
    if (options.order_by === "latest" || options.order_by === "time") {
      sql += " ORDER BY first_seen_at DESC, collected_at DESC, id DESC";
    } else {
  ...
  function getRawItemCountsByPlatform(platforms = []) {
    try {
      if (!Array.isArray(platforms) || platforms.length === 0) {
        const rows = db.prepare("SELECT platform, COUNT(*) as count FROM raw_items GROUP BY platform").all();
        return Object.fromEntries(rows.map(r => [r.platform, r.count]));
      }
      const placeholders = platforms.map(() => "?").join(",");
      const rows = db.prepare(`SELECT platform, COUNT(*) as count FROM raw_items WHERE platform IN (${placeholders}) GROUP BY platform`).all(...platforms);
      return Object.fromEntries(rows.map(r => [r.platform, r.count]));
    } catch {
      return {};
    }
