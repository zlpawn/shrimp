...
export function coarseClusterItems(items = [], focusTopics = []) {
  ...
  for (const cluster of clusters) {
    ...
    const primaryItem = (cluster.items || []).find(i => i.url) || cluster.items?.[0];
    cluster.primary_url = primaryItem?.url || primaryItem?.mobile_url || "";
    cluster.platform_urls = {};
    for (const item of cluster.items || []) {
      if (item.platform && (item.url || item.mobile_url) && !cluster.platform_urls[item.platform]) {
        cluster.platform_urls[item.platform] = item.url || item.mobile_url;
      }
    }
  }
