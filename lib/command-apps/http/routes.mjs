...
export async function routeCommandAppsRequest(req, res, _context, reqPath, { service }) {
  try {
    ...
    const installMatch = pathOnly.match(/^\/v1\/command-apps\/apps\/([^/]+)\/install$/);
    const updateMatch = pathOnly.match(/^\/v1\/command-apps\/apps\/([^/]+)\/update$/);
    ...
    if (installMatch && method === "POST") {
      await readJson(req);
      return sendJson(res, 200, await service.installLangbot(decodeURIComponent(installMatch[1])));
    }
    if (updateMatch && method === "POST") {
      await readJson(req);
      return sendJson(res, 200, await service.updateLangbot(decodeURIComponent(updateMatch[1])));
    }
