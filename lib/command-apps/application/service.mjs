...
export function createCommandAppsService({
  ...
  function renderHindsightEnvPatch(previousText, patch = {}) {
    const next = parseEnvFile(previousText);
    for (const [name, envName] of Object.entries(LLM_KEYS)) {
      if (name === "apiKey") {
        if (patch[name] !== undefined && patch[name] !== "__unchanged__" && patch[name] !== null) {
    ...
    return renderEnvFile(next, previousText);
  }
  ...
  async function safelyStopHindsightProfile(app, profileName) {
    ...
    const lockPid = readHindsightLockPid({
    ...
    if (inspected.status === "running") {
      try {
        ...
        // Continue to the ownership-checked fallback below.
      }
    }
  ...
  async function applyHindsightRuntimeConfig(app, patch) {
    ...
    try {
      ...
      if (!verifiedPids.some((pid) => isPidAlive(pid))) {
  ...
  return {
    ...
    async updateConfig(appId, patch = {}) {
      ...
      if (app.type === "cli-daemon" && app.daemonKind !== "langbot" && hasHindsightRuntimePatch(patch)) {
        return withHindsightLifecycle(() => applyHindsightRuntimeConfig.call(this, app, patch));
      }
      const current = settingsFor(app);
