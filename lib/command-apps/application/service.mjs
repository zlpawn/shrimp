    ...
    async updateConfig(appId, patch = {}) {
      ...
      if (app.type === "cli-daemon" && hasSourcePatch) {
        const profiles = {
          ...
          default: {
            ...config.hindsightProfiles.default,
            llmSource: patch.llmSource !== undefined ? patch.llmSource : config.hindsightProfiles.default.llmSource,
            embeddingSource: patch.embeddingSource !== undefined ? patch.embeddingSource : config.hindsightProfiles.default.embeddingSource,
          },
        };
        config = {
          ...config,
        ...
        const sourcePatch = renderHindsightSources(config, "default");
        if (sourcePatch) writeHindsightLlm(sourcePatch);
        saveConfig(config);
