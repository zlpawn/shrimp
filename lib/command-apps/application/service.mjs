...
import { listUnixProcesses, terminateUnixProcess as defaultTerminateUnixProcess } from "../infra/unix-processes.mjs";
import { voicestudioUrl, probeVoiceStudioHealth } from "../infra/voicestudio.mjs";
...
function statusFrom(app, settings, matches = [], managed = null, platform = process.platform, error = null, extras = {}) {
  ...
  const urls = app.id === "voicestudio"
    ? voicestudioUrl(app, settings)
    : app.type === "cli-daemon"
  ...
  return {
    ...
    healthy: extras.healthy !== undefined ? extras.healthy : null,
    llm: extras.llm || null,
...
export function createCommandAppsService({
  ...
  terminateProcess,
  terminateUnixProcess = defaultTerminateUnixProcess,
  spawnProcess = spawn,
  fileExists = (value) => fs.existsSync(value),
  readFile = (p) => fs.readFileSync(p, "utf8"),
  listHindsightProfiles,
  listHindsightProcesses = (options) => listHindsightProcessDetails(options),
  readHindsightLock = (options) => readHindsightLockPid(options),
  readHindsightLlm,
  readHindsightEnvFile = (filePath) => (fileExists(filePath) ? readFile(filePath) : ""),
  writeHindsightEnvFile = (filePath, content) => fs.writeFileSync(filePath, content, { encoding: "utf8", mode: 0o600 }),
  writeHindsightLlm,
  readCodingAgentPlugin,
  writeCodingAgentPlugin,
  ...
  inspectLangbot = (app, settings, options = {}) => inspectLangBotDaemon(app, settings, { probe: probeLangbot, ...options }),
  ...
  probeVoicestudio = (app, settings) => probeVoiceStudioHealth(app, settings),
  ...
  const listHindsightProfilesResolved = listHindsightProfiles
    || ((options = {}) => listHindsightProfileFiles({ homeDir: resolvedHomeDir, ...options }));
  const readHindsightLlmResolved = readHindsightLlm
    || ((profileName = "default") => publicLlmConfig(readHindsightConfig({ homeDir: resolvedHomeDir, profileName })));
  const writeHindsightLlmResolved = writeHindsightLlm
    || ((patch, profileName = "default") => writeHindsightLlmConfig(patch, { homeDir: resolvedHomeDir, profileName }));
  const readCodingAgentPluginResolved = readCodingAgentPlugin
    || (() => readCodingAgentPluginConfig({ homeDir: resolvedHomeDir }));
  const writeCodingAgentPluginResolved = writeCodingAgentPlugin
    || ((patch) => writeCodingAgentPluginConfig(patch, { homeDir: resolvedHomeDir }));
  ...
  function discoveredProfiles() {
    try {
      return listHindsightProfilesResolved();
    } catch {
      return [{ name: "default", configPath: defaultHindsightConfigPath(resolvedHomeDir), port: 8888 }];
    }
  }
  ...
  function llmFor(app, profileName = "default") {
    ...
    try {
      return readHindsightLlmResolved(profileName);
    } catch {
      return {
        provider: "",
        baseUrl: "",
  ...
  function pluginBinding(profileName = "default") {
    try {
      const plugin = readCodingAgentPluginResolved();
  ...
  function hindsightRuntimeSettings(app, profileName = "default") {
    const settings = settingsWithProfile(app, profileName);
    try {
      const llm = readHindsightLlmResolved(profileName);
      const port = Number(llm?.port);
      if (Number.isInteger(port) && port > 0 && port < 65536) return { ...settings, port };
    } catch {}
    return settings;
  }

  function hasHindsightRuntimePatch(patch = {}) {
    return Boolean(
      patch.llm
      || patch.llmSource !== undefined
  ...
  async function safelyStopHindsightProfile(app, profileName) {
    const settings = hindsightRuntimeSettings(app, profileName);
    const inspected = await inspectHindsight(app, settings);
    if (inspected.status === "stopped") return { wasActive: false };

    const lockPid = readHindsightLock({
      lockPath: defaultHindsightLockPath(undefined, profileName),
      fileExists,
      readFile,
    });
    const processes = await listHindsightProcesses({ platform });
    const profileProcessPids = processes
      .filter((row) => commandLineMatchesHindsightProfile(row?.commandLine || row?.command, profileName))
      .map((row) => Number(row?.pid))
      .filter((pid) => Number.isInteger(pid) && pid > 1);
    const safePids = findSafelyTerminableHindsightPids({
      profileName,
      processes,
      lockPid,
      executablePath: settings.executablePath,
    ...
    const stopFailure = (message) => new CommandAppsError("process_error", message, {
      configSaved: false,
    ...
    const knownPid = inspected.pid || lockPid;
    if (knownPid && !safePids.includes(knownPid) && isPidAlive(knownPid)) {
      throw stopFailure("Hindsight process identity could not be verified before saving configuration");
    }

    let endpointHealthy = await probeHindsight(app, settings);
    if (
      inspected.status === "launching"
      || stopError
      || endpointHealthy
      || safePids.some((pid) => isPidAlive(pid))
      || profileProcessPids.some((pid) => isPidAlive(pid))
    ) {
      const currentProcesses = await listHindsightProcesses({ platform });
      const currentProfilePids = currentProcesses
        .filter((row) => commandLineMatchesHindsightProfile(row?.commandLine || row?.command, profileName))
        .map((row) => Number(row?.pid))
        .filter((pid) => Number.isInteger(pid) && pid > 1 && isPidAlive(pid));
      const currentSafePids = findSafelyTerminableHindsightPids({
        profileName,
      ...
      const unverifiedAlivePids = safePids.filter((pid) => (
        isPidAlive(pid) && !currentSafePids.includes(pid)
      ));
      if (unverifiedAlivePids.length) {
        throw stopFailure("Hindsight process identity could not be re-verified before forced termination");
      }
      if (currentProfilePids.some((pid) => !currentSafePids.includes(pid))) {
        throw stopFailure("Hindsight profile process remained alive without a verifiable profile lock");
      }
      if (knownPid && !currentSafePids.includes(knownPid) && isPidAlive(knownPid)) {
        throw stopFailure("Hindsight process identity could not be verified before forced termination");
      }
      for (const pid of [...currentSafePids].reverse()) {
        if (isPidAlive(pid)) await terminateProcessByPlatform(pid);
      }
      endpointHealthy = await probeHindsight(app, settings);
      const alive = currentSafePids.filter((pid) => isPidAlive(pid));
      if (alive.length) {
        throw stopFailure("Hindsight did not fully stop before saving configuration");
      }
    }
    if (endpointHealthy) {
      throw stopFailure("Hindsight endpoint did not become unhealthy before saving configuration");
    }
    return { wasActive: true };
  }

  async function applyHindsightRuntimeConfig(app, patch) {
    const profileName = profileNameFromId(app.id);
    const stop = await safelyStopHindsightProfile(app, profileName);
    const beforeConfig = getConfig();
    const envPath = defaultHindsightConfigPath(undefined, profileName);
    const previousEnvText = readHindsightEnvFile(envPath);

    let nextConfig = beforeConfig;
    let envPatch = patch.llm || null;
    const hasSourcePatch = patch.llmSource !== undefined || patch.embeddingSource !== undefined;
    if (hasSourcePatch) {
      nextConfig = {
        ...beforeConfig,
        hindsightProfiles: {
          ...beforeConfig.hindsightProfiles,
          [profileName]: {
            ...beforeConfig.hindsightProfiles[profileName],
            displayName: beforeConfig.hindsightProfiles[profileName]?.displayName || profileName,
            llmSource: patch.llmSource !== undefined ? patch.llmSource : beforeConfig.hindsightProfiles[profileName]?.llmSource,
            embeddingSource: patch.embeddingSource !== undefined ? patch.embeddingSource : beforeConfig.hindsightProfiles[profileName]?.embeddingSource,
          },
        },
      };
      const sourcePatch = renderHindsightSources(nextConfig, profileName);
      const safeLlmPatch = envPatch ? { ...envPatch } : null;
      if (safeLlmPatch && nextConfig.hindsightProfiles[profileName]?.llmSource?.type === "gateway") {
        delete safeLlmPatch.provider;
        delete safeLlmPatch.baseUrl;
        delete safeLlmPatch.model;
        delete safeLlmPatch.apiKey;
      }
      if (safeLlmPatch && nextConfig.hindsightProfiles[profileName]?.embeddingSource?.type === "local") {
        safeLlmPatch.embeddingsProvider = "local";
        safeLlmPatch.embeddingsModel = "";
        safeLlmPatch.embeddingsApiKey = "";
      } else if (safeLlmPatch && nextConfig.hindsightProfiles[profileName]?.embeddingSource?.type === "gateway") {
        delete safeLlmPatch.embeddingsProvider;
        delete safeLlmPatch.embeddingsModel;
        delete safeLlmPatch.embeddingsApiKey;
      }
      envPatch = { ...(safeLlmPatch || {}), ...(sourcePatch || {}) };
    } else if (!envPatch) {
      envPatch = {
    ...
    const writeFailure = (configState, error) => new CommandAppsError(
      "storage_error",
      `Failed to save Hindsight configuration: ${error?.message || error}`,
      {
        configSaved: false,
        configState,
        restartRequired: stop.wasActive,
        restarted: false,
        phase: "write",
      },
    );
    try {
      writeHindsightLlmResolved(envPatch, profileName);
      saveConfig(nextConfig);
    } catch (error) {
      try {
        writeHindsightEnvFile(envPath, previousEnvText);
        saveConfig(beforeConfig);
      } catch (rollbackError) {
          throw writeFailure("unknown", error);
      }
      throw writeFailure("rolled_back", error);
    }

    if (!stop.wasActive) {
      const status = await this.getStatus(app.id);
      return { ...status, configApply: { restartRequired: false, restarted: false } };
    }

    const startFailure = (message, error = null) => new CommandAppsError("process_error", message, {
      configSaved: true,
      configState: "saved",
      restartRequired: true,
      restarted: false,
    ...
    try {
      const newSettings = hindsightRuntimeSettings(app, profileName);
      if (await probeHindsight(app, newSettings)) {
        throw new Error(`Hindsight endpoint port ${newSettings.port} is already occupied`);
      }
      const result = await startHindsight(app, newSettings, {
        spawnProcess,
        env: daemonEnv(),
        platform,
        probe: probeHindsight,
      ...
      if (result?.alreadyRunning) {
        throw new Error("Hindsight reported the new endpoint as already running");
      }
      const processes = await listHindsightProcesses({ platform });
      const lockPid = readHindsightLock({
        lockPath: defaultHindsightLockPath(undefined, profileName),
      ...
      const verifiedPids = findSafelyTerminableHindsightPids({
        profileName,
        processes,
        lockPid: lockPid || result?.pid,
        executablePath: newSettings.executablePath,
        platform,
      });
      const verifiedStartedProcess = result?.pid
        ? verifiedPids.includes(result.pid) && isPidAlive(result.pid)
        : verifiedPids.some((pid) => isPidAlive(pid));
      if (!verifiedStartedProcess) {
        throw new Error("Started Hindsight process could not be verified for the target profile");
      }
      ...
      saveConfig({
        ...
        apps: {
          ...getConfig().apps,
          [app.id]: {
            ...getConfig().apps[app.id],
            lastLaunchedAt: launchedAt,
          },
        },
      });
      const status = await this.getStatus(app.id);
      return { ...status, configApply: { restartRequired: true, restarted: true } };
    } catch (error) {
      if (error instanceof CommandAppsError && error.details?.phase === "start") throw error;
      throw startFailure(`Configuration saved, but Hindsight restart failed: ${error?.message || error}`, error);
    }
  }
  ...
  async function resolveExecutable(app, { allowDiscovery = true } = {}) {
    ...
    if (current?.executablePath) {
      return validateAppSettings(app, current, { platform, fileExists, homeDir: resolvedHomeDir });
    }
    if (!allowDiscovery) {
      return null;
    }
    ...
    return validateAppSettings(app, saved.apps[app.id], { platform, fileExists, homeDir: resolvedHomeDir });
  }
  ...
  return {
    ...
    async updateHindsightTool() {
      return withHindsightLifecycle(() => this.updateHindsightToolImpl());
    },
    ...
    async getStatus(appId) {
      ...
      const extras = daemonExtras(app, app.profileName);
      if (app.id === "voicestudio") {
        if (matches.length > 0) {
          try {
            extras.healthy = await probeVoicestudio(app, profileSettings);
          } catch {
            extras.healthy = false;
          }
        } else {
          extras.healthy = false;
        }
      }
      return statusFrom(app, profileSettings, matches, processStore.get(app.id), platform, loadError, extras);
    },

    async launchImpl(appId) {
      const app = requireApp(appId);
      if (!app.supportedPlatforms.includes(platform)) {
        throw new CommandAppsError(
          "unsupported_platform",
      ...
      if (app.type === "project") {
        ...
        if (platform === "win32") {
          ...
          child = await spawnProcess("runas.exe", [
            "/trustlevel:0x20000",
            `"${settings.executablePath}"${formattedArgs}`,
          ], {
            detached: true,
            stdio: "ignore",
            windowsHide: true,
          });
        } else if (platform === "darwin" && settings.executablePath.toLowerCase().endsWith(".app")) {
          child = await spawnProcess("open", [
            "-a",
    ...
    async launch(appId) {
      const app = requireApp(appId);
      if (app.type === "cli-daemon" && app.daemonKind !== "langbot") {
        return withHindsightLifecycle(() => this.launchImpl(appId));
      }
      return this.launchImpl(appId);
    },
    ...
    async stop(appId) {
      const app = requireApp(appId);
      if (app.type === "cli-daemon" && app.daemonKind !== "langbot") {
        return withHindsightLifecycle(() => this.stopImpl(appId));
      }
      return this.stopImpl(appId);
    },

    async updateConfigImpl(appId, patch = {}) {
      const app = requireApp(appId);
      if (patch && patch.args !== undefined) {
        throw new CommandAppsError("invalid_request", "Arguments cannot be changed");
      }
      if (app.type === "cli-daemon" && app.daemonKind !== "langbot" && hasHindsightRuntimePatch(patch)) {
        return applyHindsightRuntimeConfig.call(this, app, patch);
      }
      const current = settingsFor(app);
      let config = getConfig();
      if (app.type === "cli-daemon" && patch?.daemonProfile !== undefined) {
        writeCodingAgentPluginResolved({ daemonProfile: patch.daemonProfile });
      ...
      if (app.type === "cli-daemon" && hasSourcePatch) {
        ...
        if (sourcePatch || safeLlmPatch) writeHindsightLlmResolved({ ...(safeLlmPatch || {}), ...(sourcePatch || {}) }, name);
        saveConfig(config);
      ...
      if (app.type === "cli-daemon" && hasLlmPatch) {
        writeHindsightLlmResolved(llmPatch || {
          provider: patch.provider,
          baseUrl: patch.baseUrl,
          model: patch.model,
          apiKey: patch.apiKey,
      ...
      const validated = validateAppSettings(app, {
        ...
        lastLaunchedAt: current?.lastLaunchedAt || null,
      }, { platform, fileExists, homeDir: resolvedHomeDir });
      saveConfig({
        ...config,
        apps: {
          ...config.apps,
    ...
    async updateConfig(appId, patch = {}) {
      const app = requireApp(appId);
      if (app.type === "cli-daemon" && app.daemonKind !== "langbot") {
        return withHindsightLifecycle(() => this.updateConfigImpl(appId, patch));
      }
      return this.updateConfigImpl(appId, patch);
    },
  };
}
