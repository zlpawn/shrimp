...
import { sanitizeDaemonEnv, PROXY_ENV_KEYS } from "./hindsight-daemon.mjs";

const STARTUP_WINDOW_MS = 120000;

export function sanitizeLangBotEnv(source = process.env, overrides = {}) {
  const env = sanitizeDaemonEnv(source);
...
export async function inspectLangBotDaemon(app, settings = {}, {
  ...
  if (await probe(app, settings)) {
    ...
    return { status: "running", pid: Number.isInteger(pid) && pid > 0 ? pid : 0 };
  }
  const pid = Number(managedPid);
...
export async function startLangBotDaemon(app, settings = {}, {
  spawnProcess = spawn,
  probe = (currentApp, currentSettings) => probeLangBotHealth(currentApp, currentSettings),
  ...
  await ensureDirectories({ cwd: settings.cwd, dataRoot: settings.dataRoot });
...
function defaultEnsureDirectories({ cwd, dataRoot } = {}) {
