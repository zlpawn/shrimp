...
export function findSafelyTerminableHindsightPids({
  ...
  const executableName = platform === "win32"
    ? String(executablePath || "hindsight-embed.exe").split(/[\\/]/).pop().toLowerCase()
    : String(executablePath || "hindsight-embed").split("/").pop();
  const parents = rows.filter((row) => (
    commandLineMatchesHindsightProfile(row.commandLine, profileName)
  ));
  const safe = [];
  for (const parent of parents) {
    safe.push(parent.pid);
  ...
  return [...new Set(safe)].filter((pid) => pid !== 1);
}
