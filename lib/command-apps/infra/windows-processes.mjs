...
export function findProcessesByExecutable(
  ...
  const rawBase = pathLib.basename(String(executablePath || "")).toLowerCase();
  const expectedName = rawBase;
  const strippedName = rawBase.replace(/\.(exe|app)$/i, "");
  return (processes || []).filter((row) => {
    const current = String(row?.executablePath || "");
    if (current && normalizeExecutable(current, platform) === wanted) return true;
    const currentName = String(row?.name || "").toLowerCase();
    if (currentName && (currentName === expectedName || currentName.replace(/\.exe$/i, "") === strippedName)) return true;
    const cmd = String(row?.commandLine || "");
    if (executablePath.toLowerCase().endsWith(".app")) {
      if (cmd.toLowerCase().includes(wanted.toLowerCase())) return true;
      if (current.toLowerCase().includes(wanted.toLowerCase())) return true;
    }
    return false;
  });
}
