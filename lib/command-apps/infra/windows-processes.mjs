...
export function findProcessesByExecutable(
  ...
  const expectedName = path.basename(String(executablePath || "")).toLowerCase();
...
function parseProcessCsv(text) {
  ...
  for (const line of lines.slice(1)) {
    ...
    if (!Number.isInteger(pid) || pid <= 0 || (!path.isAbsolute(executablePath) && !name)) continue;
    rows.push({ pid, executablePath, name });
  }
  return rows;
}
