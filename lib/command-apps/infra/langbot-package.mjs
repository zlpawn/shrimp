...
export async function installedLangBotVersion({
  uvPath = findUv(),
  execFile = execFileP,
  timeoutMs = 8000,
} = {}) {
  ...
  try {
    const { stdout } = await execFile(uvPath, ["tool", "list"], {
      encoding: "utf8",
      timeout: timeoutMs,
      windowsHide: true,
      env: sanitizeLangBotEnv(),
    });
    const line = String(stdout || "").split(/\r?\n/).find((item) => /^langbot\s+v/i.test(item.trim()));
    const match = line?.match(/v([0-9]+\.[0-9]+\.[0-9]+(?:[-+][0-9A-Za-z.-]+)?)/i);
    return match?.[1] || null;
  } catch {
    return null;
  }
...
export async function upgradeLangBotTool({
  ...
  return {
    executablePath: discovered,
    version: await installedLangBotVersion({ uvPath, execFile }),
    recovered: false,
  };
}
