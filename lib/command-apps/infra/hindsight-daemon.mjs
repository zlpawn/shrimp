...
import {
  defaultHindsightLockPath as profileLockPath,
  defaultHindsightPort,
  hindsightDaemonArgs,
  normalizeHindsightProfileName,
} from "./hindsight-config.mjs";
...
const HINDSIGHT_STARTUP_WINDOW_MS = 180000;
...
export function defaultHindsightLockPath(homeDir = os.homedir(), profileName = "default") {
  return profileLockPath(homeDir, profileName);
}
...
export async function inspectHindsightDaemon(app, settings = {}, options = {}) {
  ...
  const withinStartupWindow = Number.isFinite(launchedAt)
    && Date.now() - launchedAt < HINDSIGHT_STARTUP_WINDOW_MS;
  ...
  if (pid && pidAlive && withinStartupWindow) {
    return { status: "launching", pid };
  }
  const runningPids = options.listPids ? await options.listPids() : await listHindsightPids(options);
  if (runningPids?.length && withinStartupWindow) {
    return { status: "launching", pid: runningPids[0] };
  }
  return { status: "stopped", pid: null };
}

function daemonUrl(app, settings = {}, options = {}) {
  const current = settings || {};
  const profileName = normalizeHindsightProfileName(options.profileName || current.profileName || "default");
  const configuredPort = Number(options?.llm?.port || current.port || defaultHindsightPort(profileName) || app.defaultPort || 8888);
  const port = Number.isInteger(configuredPort) && configuredPort > 0 && configuredPort < 65536
    ? configuredPort
    : (app.defaultPort || 8888);
...
export async function startHindsightDaemon(app, settings, {
  ...
  const profileName = normalizeHindsightProfileName(settings?.profileName || "default");
  const child = await spawnDaemonCommand(settings.executablePath, hindsightDaemonArgs("start", profileName), {
    spawnProcess,
    env,
    platform,
  });
...
export async function stopHindsightDaemon(app, settings, {
  ...
  const profileName = normalizeHindsightProfileName(settings?.profileName || "default");
  const child = await spawnDaemonCommand(settings.executablePath, hindsightDaemonArgs("stop", profileName), {
    spawnProcess,
    env,
    platform,
  });
