...
function wellKnownPaths(app, env = process.env, pathLib = path.win32) {
  ...
  } else if (app.id === "voicestudio") {
    if (localAppData) {
      candidates.push(pathLib.join(localAppData, "Programs", "VoiceStudio", app.executableName));
      candidates.push(pathLib.join(localAppData, "VoiceStudio", app.executableName));
      candidates.push(pathLib.join(localAppData, "Programs", "VoiceStudio", "VoiceStudio.exe"));
    }
    candidates.push(pathLib.join(programFiles, "VoiceStudio", app.executableName));
    candidates.push(pathLib.join("C:\\Program Files", "VoiceStudio", app.executableName));
    candidates.push(pathLib.join(programFilesX86, "VoiceStudio", app.executableName));
    candidates.push(pathLib.join("C:\\Program Files (x86)", "VoiceStudio", app.executableName));
    candidates.push(pathLib.join(programFiles, "VoiceStudio", "VoiceStudio.exe"));
  }
  return candidates;
}

function commonDriveRoots(app, env = process.env, pathLib = path.win32) {
  const candidates = [];
  if (app.id === "voicestudio") {
    const drives = ["D:", "C:", "E:", "F:"];
    for (const drive of drives) {
      candidates.push(pathLib.join(drive, "VoiceStudio", app.executableName));
      candidates.push(pathLib.join(drive, "VoiceStudio", "VoiceStudio.exe"));
    }
  }
  return candidates;
}

function wellKnownMacAppPaths(app, env = process.env, pathLib = path.posix) {
  const home = env.HOME || "";
  const candidates = [];
  if (app.id === "voicestudio") {
    candidates.push("/Applications/VoiceStudio.app");
    if (home) {
      candidates.push(pathLib.join(home, "Applications", "VoiceStudio.app"));
    }
  }
  return candidates;
}
...
async function defaultSearchPathDirs(env = process.env, platform = process.platform) {
  const value = env.PATH || env.Path || "";
  const sep = platform === "win32" ? ";" : (value.includes(";") && !value.includes(":") ? ";" : ":");
  return value.split(sep).map((item) => item.trim()).filter(Boolean);
}

async function defaultReadShortcutTarget(app) {
  ...
  const pattern = app?.id === "voicestudio" ? "VoiceStudio|omnivoice" : "Antigravity";
  const script = [
    ...
    `foreach ($file in $files) { if ($file.Name -match '${pattern}') { $shell.CreateShortcut($file.FullName).TargetPath } }`,
  ].join("; ");
  try {
...
async function isValidExecutable(candidate, {
  ...
  if (requireExe && pathLib.extname(candidate).toLowerCase() !== ".exe") return false;
  try {
    const stat = await statFile(candidate);
    return Boolean(stat?.isFile?.() || (candidate.endsWith(".app") && stat?.isDirectory?.()));
  } catch {
    return false;
  }
}

function cliDaemonNames(app, platform) {
  const name = String(app?.executableName || "").trim();
  if (!name) return [];
  if (platform === "win32") {
    return name.toLowerCase().endsWith(".exe") ? [name] : [name, `${name}.exe`];
  }
  return [name];
}

function wellKnownCliDaemonPaths(app, env = process.env, pathLib = path, platform = process.platform) {
  const names = cliDaemonNames(app, platform);
  const home = env.HOME || env.USERPROFILE || "";
  const localAppData = env.LOCALAPPDATA || "";
  const dirs = [];
  if (home) {
    dirs.push(pathLib.join(home, ".local", "bin"));
    dirs.push(pathLib.join(home, ".cargo", "bin"));
  }
  if (platform === "win32" && localAppData) {
    dirs.push(pathLib.join(localAppData, "uv", "bin"));
  }
  const values = [];
  for (const dir of dirs) {
    for (const name of names) values.push(pathLib.join(dir, name));
  }
  return values;
}
...
export async function discoverCommandApp(app, {
  ...
  readShortcutTarget = (targetApp) => defaultReadShortcutTarget(targetApp || app),
} = {}) {
  if (!app) {
  ...
  if (app.type === "cli-daemon") {
    const names = cliDaemonNames(app, platform);
    const pathDirs = await searchPathDirs(env, platform);
    const groups = [
      {
        strategy: "well-known-local-bin",
        values: wellKnownCliDaemonPaths(app, env, pathLib, platform),
      },
      {
        strategy: "path-environment",
        values: pathDirs.flatMap((dir) => names.map((name) => pathLib.join(dir, name))),
      },
    ];
    const seen = new Set();
    const candidates = [];
    for (const group of groups) {
      for (const raw of group.values) {
        const candidate = normalizeCandidate(raw, group.strategy, "local-cli", pathLib);
        if (!candidate) continue;
        const key = candidate.path.toLowerCase();
        if (seen.has(key)) continue;
        if (!(await isValidExecutable(candidate.path, { fileExists, statFile, pathLib, requireExe: false }))) continue;
        seen.add(key);
        candidates.push(candidate);
      }
    }
  ...
  const groups = [];
  if (platform === "win32") {
    const rawShortcuts = await readShortcutTarget(app);
    const shortcutValues = Array.isArray(rawShortcuts) ? rawShortcuts : [rawShortcuts];
    if (app.id === "antigravity") {
      groups.push({
      ...
      groups.push({
        strategy: "path-environment",
        values: (await searchPathDirs(env, platform)).map((dir) => pathLib.join(dir, app.executableName)),
      });
      groups.push({
      ...
      groups.push({
        strategy: "common-drive-roots",
        values: commonDriveRoots(app, env, pathLib),
      });
      groups.push({
        strategy: "windows-app-paths",
        values: [
          ...(await queryAppPaths(app.executableName)),
          ...(app.id === "voicestudio" ? await queryAppPaths("VoiceStudio.exe") : []),
        ],
      });
      groups.push({
        strategy: "path-environment",
        values: [
          ...(await searchPathDirs(env, platform)).map((dir) => pathLib.join(dir, app.executableName)),
          ...(app.id === "voicestudio" ? (await searchPathDirs(env, platform)).map((dir) => pathLib.join(dir, "VoiceStudio.exe")) : []),
        ],
      });
    }
  } else if (platform === "darwin") {
    groups.push({
      strategy: "macos-applications",
      values: wellKnownMacAppPaths(app, env, pathLib),
    });
    groups.push({
      strategy: "path-environment",
      values: [
        ...(await searchPathDirs(env, platform)).map((dir) => pathLib.join(dir, app.executableName)),
        ...(app.macExecutableName ? (await searchPathDirs(env, platform)).map((dir) => pathLib.join(dir, app.macExecutableName)) : []),
      ],
    });
  }
  ...
  for (const group of groups) {
    for (const raw of group.values) {
      ...
      const key = candidate.path.toLowerCase();
      if (seen.has(key)) continue;
      if (!(await isValidExecutable(candidate.path, { fileExists, statFile, pathLib, requireExe: platform === "win32" }))) continue;
      seen.add(key);
      candidates.push(candidate);
    }
  }
