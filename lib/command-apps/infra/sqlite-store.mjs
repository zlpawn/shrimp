...
export function createCommandAppsSqliteStore({
  ...
  const settingsColumns = db.prepare("PRAGMA table_info(command_apps_settings)").all();
  if (!settingsColumns.some((column) => column.name === "daemon_json")) {
    db.exec("ALTER TABLE command_apps_settings ADD COLUMN daemon_json TEXT;");
  }
