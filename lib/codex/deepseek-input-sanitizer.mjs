...
export const DEEPSEEK_AGENT_EXECUTION_RULES =
  "[Agent Execution Rules for DeepSeek]\n" +
  "1. Multi-Step Execution: In multi-step tasks, you MUST NOT stop after a stage summary or next-step plan.\n" +
  "2. Immediate Action: If you state what to do next (e.g. \"接下来...\", \"下一步...\"), you MUST invoke the corresponding tool IMMEDIATELY in the same response turn.\n" +
  "3. No Empty Announcements: Do not announce actions in natural language without executing the tool call in the same turn.";

export function injectDeepSeekAgentRules(input, instructions = null) {
  let updatedInstructions = instructions;
  if (typeof instructions === "string" && instructions.trim()) {
    if (!instructions.includes("[Agent Execution Rules for DeepSeek]")) {
      updatedInstructions = `${instructions.trim()}\n\n${DEEPSEEK_AGENT_EXECUTION_RULES}`;
    }
  }

  if (!Array.isArray(input) || input.length === 0) {
    return { input, instructions: updatedInstructions };
  }

  let injected = typeof updatedInstructions === "string" && updatedInstructions.includes("[Agent Execution Rules for DeepSeek]");
  const newInput = input.map((item) => {
    if (!item || typeof item !== "object") return item;
    if (
      !injected
      && (item.role === "system" || item.role === "developer" || (item.type === "message" && (item.role === "system" || item.role === "developer")))
    ) {
      injected = true;
      if (typeof item.content === "string") {
        if (!item.content.includes("[Agent Execution Rules for DeepSeek]")) {
          return { ...item, content: `${item.content.trim()}\n\n${DEEPSEEK_AGENT_EXECUTION_RULES}` };
        }
      } else if (Array.isArray(item.content)) {
        const textParts = item.content.map((p) => p?.text || "").join("");
        if (!textParts.includes("[Agent Execution Rules for DeepSeek]")) {
          return {
            ...item,
            content: [
              ...item.content,
  ...
  if (!injected && (typeof updatedInstructions !== "string" || !updatedInstructions.includes("[Agent Execution Rules for DeepSeek]"))) {
    newInput.unshift({
      type: "message",
      role: "system",
      content: [{ type: "input_text", text: DEEPSEEK_AGENT_EXECUTION_RULES }],
    });
  }

  return { input: newInput, instructions: updatedInstructions };
}

export function sanitizeDeepSeekResponsesInput(source, { injectAgentRules = true } = {}) {
  ...
  let reorderedInput = reorderToolCallsWithOutputs(cleaned.input, reasoningQueue);

  const hasTools = Array.isArray(source.tools) && source.tools.length > 0;
  if (hasTools && injectAgentRules !== false) {
    const injected = injectDeepSeekAgentRules(reorderedInput, cleaned.instructions);
    reorderedInput = injected.input;
    if (injected.instructions) {
      cleaned.instructions = injected.instructions;
    }
  }
