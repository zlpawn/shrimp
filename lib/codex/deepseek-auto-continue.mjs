...
export const DEFAULT_DEEPSEEK_AUTO_CONTINUE_PROMPT =
  "请立即直接调用工具执行下一步操作，禁止输出自然语言确认、开场白或阶段性总结。";
...
const CONTINUE_HINTS = [
  /现在(?:进入|做|开始|处理|修改|执行|查看)/,
  /接下来(?:我将|我们将|我们需要|先|准备|会|去)?[^\n，。]{0,6}(?:进入|做|执行|开始|处理|检查|查看|运行|修改|写|读|查|调|测|创建|分析|推进)/,
  /下面(?:我将|我们将|我们需要|先|准备|会|去)?[^\n，。]{0,6}(?:进入|做|执行|开始|处理|检查|查看|运行|修改|写|读|查|调|测|创建|分析|推进)/,
  /下一步(?:我将|我们将|我们需要|是|：|:|做|执行|开始|处理|修改|写|查|测)?/,
  /继续(?:我将|我们将|我们需要|先|去)?[^\n，。]{0,6}(?:执行|做|处理|检查|完成|推进|修改|运行|测试)/,
  /先看(?:一下|下|代码|文件|目录|实现)?/,
  /先做/,
  /现在做/,
  /开始做/,
  /(?:准备|马上|这就)(?:进入|做|执行|开始|处理|修改|写|查|测|运行)/,
  /Task\s*\d+/i,
  /Plan\s*\d+/i,
  /let me (?:check|look|run|read|open|inspect|continue|start|do|implement|fix|modify|create|test)/i,
  /i(?:'| a)m going to (?:check|look|run|read|open|inspect|continue|start|do|implement|fix|modify|create|test|proceed)/i,
  /i will (?:now )?(?:check|look|run|read|open|inspect|continue|start|do|implement|fix|modify|create|test|proceed)/i,
  /let(?:'s| us) (?:proceed|move on|start|continue|check|look|run|read|open|inspect|do|implement|fix|modify|create|test)/i,
  /proceeding to (?:check|look|run|read|open|inspect|continue|start|do|implement|fix|modify|create|test)/i,
  /now (?:enter|start|do|look|check|run|read|implement|fix|proceed)/i,
];
...
export function resolveDeepSeekAutoContinueSettings({
  ...
  let injectAgentRules = fromTools.inject_agent_rules !== false;
  ...
  return {
    ...
    prompt,
    inject_agent_rules: injectAgentRules,
  };
}
...
function findLastMatch(text, regexList) {
  let lastIndex = -1;
  let found = false;
...
export function looksLikeDeepSeekMidTaskStop(text) {
  ...
  // Evaluate tail window (last 350 characters) first
  const tailLength = Math.min(value.length, 350);
  const tail = value.slice(value.length - tailLength);

  const tailContinue = findLastMatch(tail, CONTINUE_HINTS);
  const tailDone = findLastMatch(tail, DONE_HINTS);

  if (tailContinue.found) {
    if (!tailDone.found) return true;
    return tailContinue.lastIndex >= tailDone.lastIndex;
  }

  // Fallback to full text if not found in tail
  const fullContinue = findLastMatch(value, CONTINUE_HINTS);
  if (!fullContinue.found) return false;

  const fullDone = findLastMatch(value, DONE_HINTS);
  if (!fullDone.found) return true;

  return fullContinue.lastIndex >= fullDone.lastIndex;
}
...
export function accumulateUsage(prev, next) {
  if (!prev && !next) return null;
  const p = prev || {};
  const n = next || {};
  const inputTokens = (p.input_tokens || p.prompt_tokens || 0) + (n.input_tokens || n.prompt_tokens || 0);
  const outputTokens = (p.output_tokens || p.completion_tokens || 0) + (n.output_tokens || n.completion_tokens || 0);
  const totalTokens = (p.total_tokens || 0) + (n.total_tokens || 0) || (inputTokens + outputTokens);
...
export function attachUsage(response, usage) {
...
export function mergePreservedStageText(firstResponse, nextResponse) {
  ...
  let insertIndex = 0;
  while (insertIndex < output.length && output[insertIndex]?.type === "reasoning") {
    insertIndex++;
  }
  output.splice(insertIndex, 0, stageMessage);
...
export async function runDeepSeekAutoContinueLoop({
  ...
  let attempts = 0;
  let accumulatedUsage = accumulateUsage(null, response?.usage);
  ...
  while (true) {
    ...
    if (!decision.ok) {
      ...
      if (attempts > 0) {
        // If continue attempts failed to elicit tool calls, keep firstResponse
        // to avoid duplicate/repetitive text polluting downstream conversation history.
        finalResponse = firstResponse;
      }
      finalResponse = attachUsage(finalResponse, accumulatedUsage);
    ...
    if (!nextResponse) {
      return {
        body: nextBody,
        response: attachUsage(currentResponse, accumulatedUsage),
        attempts,
    ...
    accumulatedUsage = accumulateUsage(accumulatedUsage, nextResponse?.usage);
    currentBody = nextBody;
    currentResponse = nextResponse;

    if (responseHasToolCalls(currentResponse)) {
      let finalResponse = preserveStageText
