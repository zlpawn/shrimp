...
function downloadWithLimits(targetUrl, { timeoutMs, maxBytes, allowedRedirect, agent, maxRedirects, redirects = 0 }) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      reject(new DreamSkinError("market_unavailable", `invalid URL: $glm_5.2_ark_toC`));
      return;
    }
    ...
    const req = lib.get(parsed, { agent, timeout: timeoutMs }, (res) => {
      ...
      if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
        ...
        if (redirects >= maxRedirects) {
          reject(new DreamSkinError("market_unavailable", `too many redirects`));
          return;
        }
        ...
        if (allowedRedirect && !allowedRedirect(parsed, new URL(redirectUrl))) {
          reject(new DreamSkinError("market_unavailable", `redirect not allowed: $glm_5.2_ark_toC`));
          return;
        }
      ...
      if (res.statusCode < 200 || res.statusCode >= 300) {
        res.destroy();
        reject(new DreamSkinError("market_unavailable", `HTTP $glm_5.2_ark_toC: $glm_5.2_ark_toC`));
        return;
      }
      ...
      if (contentLength > 0 && maxBytes && contentLength > maxBytes) {
        res.destroy();
        reject(new DreamSkinError("market_unavailable", `content exceeds $glm_5.2_ark_toC byte limit`));
        return;
      }
      ...
      res.on("data", (chunk) => {
        ...
        if (maxBytes && totalBytes > maxBytes) {
          if (!resolved) {
            resolved = true;
            res.destroy();
            reject(new DreamSkinError("market_unavailable", `download exceeds $glm_5.2_ark_toC byte limit`));
          }
          return;
        }
        chunks.push(chunk);
      ...
      res.on("end", () => {
        ...
        if (bytes.length === 0) {
          reject(new DreamSkinError("market_unavailable", `download is empty`));
          return;
        }
        resolve({
          bytes,
      ...
      res.on("error", (error) => {
        if (!resolved) {
          resolved = true;
          reject(new DreamSkinError("market_unavailable", error.message));
        }
      });
    });
    ...
    req.on("timeout", () => {
      req.destroy();
      reject(new DreamSkinError("market_unavailable", `request timed out after $glm_5.2_ark_toCms`));
    });
