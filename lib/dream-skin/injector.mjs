...
(() => {
  ...
  // Guard against double-injection. If already installed, re-apply theme
  // in case it changed (hot-update path).
  ...
  // MutationObserver instance, kept so cleanup can disconnect it.
  let observer = null;
  ...
  function applyTheme(theme, artDataUri) {
    ...
    // pointer-events: none ensures the layer never intercepts clicks.
    // z-index: -1 places it behind all normal content.
  ...
  // CodexPlusPlus uses the same approach (dream_skin_skin_api_bootstrap_script).
  const PART_MAP = {
    root: "html",
  ...
  // CodexPlusPlus uses the same pattern with childList + subtree.
  ...
  // Cleanup function: disconnect observer, remove injected elements,
  // clear CSS variables, and reset the installed flag.
  window.__SHRIMP_DREAM_SKIN_CLEANUP__ = function() {
    if (observer) {
      observer.disconnect();
      observer = null;
    }
    ...
    for (const name of Object.keys(root.style)) {
      if (name.startsWith("--ds-")) root.style.removeProperty(name);
    }
    document.getElementById(STYLE_TAG_ID)?.remove();
    document.getElementById(CHROME_LAYER_ID)?.remove();
    ...
    window.__SHRIMP_DREAM_SKIN_APPLY__ = null;
    window.__SHRIMP_DREAM_SKIN_CLEANUP__ = null;
  };
...
(() => {
  ...
  // Fallback: manually remove everything if the cleanup function isn't registered
  // (e.g. if the injection script never ran but style tags exist).
  const root = document.documentElement;
