...
function waitForSpawn(spawn, executable, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const rejectOnce = (error) => {
      if (settled) return;
      settled = true;
      child.removeListener?.("spawn", resolveOnce);
      reject(error);
    };
    child.once("spawn", resolveOnce);
...
export function createCodexLauncher({
  ...
  async function quit(appPath) {
    if (platform === "win32") {
      ...
      await waitForSpawn(spawn, cmd.executable, cmd.args, { windowsHide: true });
      return;
    }
    ...
    await waitForSpawn(spawn, cmd.executable, cmd.args);
  }
  ...
  async function launchWithDebugPort({ appPath = "", debugPort = DEFAULT_DEBUG_PORT, extraArgs = [], maxWaitMs = 20000, allowRestart = false } = {}) {
    ...
    if (platform === "darwin") {
      ...
      const child = await waitForSpawn(spawn, cmd.executable, cmd.args, { stdio: "ignore", detached: false });
    ...
    if (resolvedAppPath.includes("WindowsApps") || resolvedAppPath.includes("!App")) {
      ...
      if (packagedRunning) {
        ...
        await waitForSpawn(spawn, quitCmd.executable, quitCmd.args, { windowsHide: true });
    ...
    const child = await waitForSpawn(spawn, cmd.executable, cmd.args, { stdio: "ignore", detached: false, windowsHide: true });
