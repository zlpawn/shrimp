...
export function createCodexLauncher({
  ...
  async function launchWithDebugPort({ appPath = "", debugPort = DEFAULT_DEBUG_PORT, extraArgs = [], maxWaitMs = 20000 } = {}) {
    ...
    if (platform === "darwin") {
      ...
      if (running) {
        // Never force-quit a running Codex just to inject. The user may be
        // actively working; ask them to restart with the debug flag instead.
        throw new DreamSkinError(
    ...
    if (resolvedAppPath.includes("WindowsApps") || resolvedAppPath.includes("!App")) {
      ...
      return launchWindowsPackaged({ appUserModelId, debugPort, extraArgs });
    }
  ...
  return { resolveCodexAppPath, isRunning, quit, launchWithDebugPort, launchWindowsPackaged };
}
