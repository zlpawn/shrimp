...
function packagedProcessPathPattern(appUserModelId) {
  ...
  return `C:\\Program Files\\WindowsApps\\${identity}_*__${publisherId}*`;
}

export function buildWindowsPackagedProcessQuery({ appUserModelId }) {
  if (!appUserModelId || typeof appUserModelId !== "string") {
    throw new DreamSkinError("invalid_request", "appUserModelId is required");
  }
  const pattern = escapePsSingleQuoted(packagedProcessPathPattern(appUserModelId));
  return {
    executable: "powershell.exe",
    args: [
      "-NoProfile", "-NonInteractive", "-WindowStyle", "Hidden",
...
export function buildWindowsPackagedQuitCommand({ appUserModelId }) {
  if (!appUserModelId || typeof appUserModelId !== "string") {
    throw new DreamSkinError("invalid_request", "appUserModelId is required");
  }
  const pattern = escapePsSingleQuoted(packagedProcessPathPattern(appUserModelId));
...
export function createCodexLauncher({
  ...
  async function isPackagedAppRunning(appUserModelId) {
    ...
    const cmd = buildWindowsPackagedProcessQuery({ appUserModelId });
    const result = await spawnSync(cmd.executable, cmd.args);
    return Boolean(result && result.stdout && Number(result.stdout.trim()) > 0);
  }
  ...
  async function launchWithDebugPort({ appPath = "", debugPort = DEFAULT_DEBUG_PORT, extraArgs = [], maxWaitMs = 20000, allowRestart = false } = {}) {
    ...
    if (resolvedAppPath.includes("WindowsApps") || resolvedAppPath.includes("!App")) {
      ...
      if (packagedRunning) {
        ...
        const quitCmd = buildWindowsPackagedQuitCommand({ appUserModelId });
        await spawn(quitCmd.executable, quitCmd.args, { windowsHide: true });
        await waitForStopped(
          () => isPackagedAppRunning(appUserModelId).catch(() => false),
