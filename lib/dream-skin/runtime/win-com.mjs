/**
 * Windows packaged app activation via PowerShell + COM.
 *
 * Activates MS Store packaged Codex apps (OpenAI.Codex, OpenAI.ChatGPT-Desktop)
 * with --remote-debugging-port through IApplicationActivationManager so the
 * arguments actually reach the packaged process. PowerShell runs fully hidden
 * (no console window flashes to the user).
 *
