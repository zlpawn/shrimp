...
function comparePackageEntriesNewestFirst(a, b) {
  const aVersion = String(a).match(/_(\d+(?:\.\d+)+)_/);
  const bVersion = String(b).match(/_(\d+(?:\.\d+)+)_/);
  if (aVersion && bVersion) {
    const aParts = aVersion[1].split(".").map(Number);
    const bParts = bVersion[1].split(".").map(Number);
...
export async function resolveWildcardPath(path) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const matches = entries.filter((entry) => regex.test(entry)).sort(comparePackageEntriesNewestFirst);
...
export function supportsDreamSkinRuntime(platform = process.platform) {
...
export function createDreamSkinApplier({
  ...
  codexAppPath = "",
  createSession = ({ wsUrl }) => createCdpSession({ wsUrl }),
  logger = console,
} = {}) {
  const supported = supportsDreamSkinRuntime(platform);
  ...
  async function probe(port = debugPort) {
    ...
    try {
      ...
      return injectable.length > 0
        ? { available: true, targets: injectable }
  ...
  async function applyTheme({
    ...
    if (!supported) {
      throw new DreamSkinError(
        "unsupported_feature",
        `Codex 运行时注入仅支持 macOS 和 Windows（当前平台：${platform}）。`,
      );
    }
    ...
    const targetKey = target.webSocketDebuggerUrl;
    const session = createSession({ wsUrl: targetKey });
    await session.connect();
    try {
      const previousIdentifier = registeredScripts.get(targetKey);
      if (previousIdentifier) {
        try {
          await session.removeScriptFromNewDocuments(previousIdentifier);
        } catch {
          // The target may belong to a restarted Codex process, where the old
          // identifier no longer exists. Registration of the new script can continue.
        }
        registeredScripts.delete(targetKey);
      }
      const registration = await session.addScriptToNewDocuments(script);
      if (registration?.identifier) {
        registeredScripts.set(targetKey, registration.identifier);
      }
  ...
  return { supported, probe, applyTheme, launcher, targetClient };
}
