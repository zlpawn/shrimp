...
import { launchCodexWithDebugPort, quitCodexApp, DEFAULT_DEBUG_PORT } from "./launcher.mjs";
...
async function injectSkin({ themePath, appPath, debugPort }) {
  ...
  if (backgroundDataUri) {
    console.log(`[dream-skin] background image loaded (${Math.round(backgroundDataUri.length / 1024)}KB data URI)`);
  }
  ...
  // Execute immediately on the current page.
  ...
  // Keep the session alive so the script persists. The launcher process
  // stays running until the Codex app is closed.
  console.log(`[dream-skin] skin is live. Press Ctrl+C to detach (Codex keeps running).`);
