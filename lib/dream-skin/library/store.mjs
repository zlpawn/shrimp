...
export function createThemeLibrary({
  ...
  async function createTheme({ theme, imageBytes }) {
    return mutationQueue.run(async () => {
      // Blank-template create: allocate a slug id from the theme name when none
      // is supplied (design 8.3). Collisions get -2, -3 suffixes.
      let themed = theme;
      const rawId = theme?.id?.trim?.();
      if (!rawId) {
        const existingIds = new Set([BUILTIN_ID]);
        try {
          const entries = await fs.promises.readdir(paths.themesDir);
          for (const entry of entries) existingIds.add(entry);
        } catch {
          // themes dir may not exist yet
        }
        const newId = allocateThemeId(theme?.name || "theme", (checkId) => existingIds.has(checkId));
        themed = { ...theme, id: newId };
      }
      return putStoredTheme({ theme: themed, imageBytes, replace: false });
    });
  }
