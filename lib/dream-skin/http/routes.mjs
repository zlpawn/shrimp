...
const STATUS_BY_CODE = {
  ...
  runtime_restart_required: 409,
  market_unavailable: 503,
  market_manifest_invalid: 502,
  market_asset_invalid: 502,
  hash_mismatch: 502,
  runtime_inject_failed: 500,
  runtime_launch_failed: 500,
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (chunk) => {
      ...
      if (totalBytes > maxBytes) {
        req.destroy();
        reject(new DreamSkinError("payload_too_large", `\u8BF7\u6C42\u4F53\u8D85\u8FC7 ${maxBytes} \u5B57\u8282\u9650\u5236\u3002`));
        return;
      }
      chunks.push(chunk);
    });
...
export async function routeDreamSkinRequest(req, res, context, reqPath, { service }) {
  ...
  // Ensure directories/state exist on first request (server.js never calls initialize explicitly)
  if (typeof service.initialize === "function") {
    await service.initialize();
  }

  // reqPath may include a query string in unit tests; strip it so route
  // matching stays stable while req.url still carries restart etc.
  const cleanPath = String(reqPath || "").split("?")[0];
  const pathParts = cleanPath.split("/").filter(Boolean); // ["v1", "dream-skin", ...]
  ...
  try {
    ...
    // GET /v1/dream-skin/settings
    if (method === "GET" && pathParts[2] === "settings" && pathParts.length === 3) {
    ...
    // PUT /v1/dream-skin/settings
    if (method === "PUT" && pathParts[2] === "settings" && pathParts.length === 3) {
    ...
    // GET /v1/dream-skin/probe
    if (method === "GET" && pathParts[2] === "probe" && pathParts.length === 3) {
    ...
    // POST /v1/dream-skin/themes/:id/apply[?restart=1]
    if (method === "POST" && pathParts[2] === "themes" && pathParts.length === 5 && pathParts[4] === "apply") {
      const id = decodeURIComponent(pathParts[3]);
      const restart = /[?&]restart=(1|true)/i.test(req.url || "");
      const result = await service.applyTheme(id, { restart });
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify(result));
      return;
    }
