...
import { retrieveAntigravityQuotaViaGrpc } from "./quota.mjs";
...
export const antigravitySubscriptionAuthProvider = {
  ...
  async getUsage(options = {}) {
    try {
      const creds = getClientCredentials(options.env || process.env);
      const fresh = await ensureFreshToken({
        store: {
          getStoredToken: () => getStoredToken(options.env || process.env),
          saveSecrets: (secrets) => saveSecrets(secrets, options.env || process.env),
        },
        clientId: creds.client_id,
        clientSecret: creds.client_secret,
      });
      return await retrieveAntigravityQuotaViaGrpc({
        accessToken: fresh.access_token,
        proxyUrl: options.proxyUrl || null,
        fallback: getAntigravityUsage,
      });
    } catch (error) {
      const fallback = getAntigravityUsage();
      if (fallback.available) return fallback;
      return {
        ...fallback,
        error: {
          code: "antigravity_quota_failed",
