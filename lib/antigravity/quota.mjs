...
function localizedLimit(group) {
  const id = group.window === "weekly" ? "weekly" : group.window;
  const label = id === "weekly" ? "周额度" : id === "5h" ? "5 小时额度" : group.title || id;
  const remainingPercent = Math.max(0, Math.min(100, Number(group.quota?.field1 ?? 0) / 10_000));
  let resetHint = group.description || "";
  if (id === "weekly") {
    resetHint = resetHint
      .replace("You have used some of your weekly limit, it will fully refresh in", "已使用部分周额度，将在")
      .replace(" days,", " 天")
      .replace(" hours.", " 小时后刷新。");
  } else if (id === "5h") {
    resetHint = resetHint
      .replace("You have used some of your 5-hour limit, it will fully refresh in", "已使用部分 5 小时额度，将在")
      .replace(/ (\d+) hour,/, " $1 小时")
      .replace(/ (\d+) hours,/, " $1 小时")
      .replace(" hours,", " 小时")
      .replace(" minutes.", " 分钟后刷新。");
  }
  return {
    id,
    label,
...
export function decodeUserQuotaSummaryForTests(payload) {
  const limits = (payload.groups || [])
    .filter((group) => group.id === "gemini-weekly" || group.window === "weekly" || group.id === "gemini-5h" || group.window === "5h")
    .map(localizedLimit);
  if (!limits.length) return normalizeAntigravityQuotaResponse(payload);
  return {
    available: true,
    remaining_percent: limits.find((limit) => limit.id === "weekly")?.remaining_percent ?? limits[0].remaining_percent,
    remaining_credits: null,
    consumed_credits: null,
    limits,
    reset_hint: limits.find((limit) => limit.id === "weekly")?.reset_hint || "",
    updated_at: new Date().toISOString(),
    error: null,
  };
}
...
export const quotaMethodRegistry = [
  {
    ...
    normalize: decodeUserQuotaSummaryForTests,
  },
  {
