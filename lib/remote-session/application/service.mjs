...
export function createRemoteSessionService({
    ...
    if (!host) {
      if (peerId === "local-host" || peerId === "fake-host") {
        ...
        if (targetPeer?.auth?.type === "ssh") {
          ...
          const token = targetPeer?.auth?.gatewayToken || "";
          host = createPeerHostProxy(createPeerClient({ baseUrl, token, fetchImpl }));
        } else {
          throw new RemoteSessionError(
            "unsupported_feature",
  ...
  async function upsertPeer(peerPatch = {}) {
    ...
    await updateConfig({ enabled: true, peers: nextPeers });
  ...
  async function dispatchPrompt({
    ...
    const events = Array.isArray(result.events) && result.events.length > 0
      ? result.events
      : [{ type: "prompt_dispatched", text: prompt, summary: prompt.slice(0, 100) }];
