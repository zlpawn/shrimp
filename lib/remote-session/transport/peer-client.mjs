...
export function createPeerClient({
  ...
  return {
    ...
    listConversations({ limit = 20 } = {}) {
      return request(
        "GET",
        "/v1/remote-session/host/conversations?limit=" + Number(limit || 20),
      );
    },
    getConversation(conversationId) {
      return request(
        "GET",
        "/v1/remote-session/host/conversations/" + encodeURIComponent(conversationId),
      );
    },
    attach() {
      return request("POST", "/v1/remote-session/host/attach", {});
    },
    createConversation(projectId, options = {}) {
      return request("POST", "/v1/remote-session/host/conversations", {
        projectId,
        model: options.model,
    ...
    dispatchPrompt({ conversationId, prompt, controllerPeerId, model, modelAlias, cascadeConfig }) {
      return request(
        "POST",
        "/v1/remote-session/host/conversations/" + encodeURIComponent(conversationId) + "/prompt",
        { prompt, controllerPeerId, model, modelAlias, cascadeConfig },
      );
    },
...
export function createPeerHostProxy(client) {
  return {
    ...
    async listConversations(args = {}) {
      const data = await client.listConversations(args);
      return data.conversations || data || [];
    },
    async createConversation(projectId, options) {
      return client.createConversation(projectId, options);
    },
