...
export function summarizePartialHostSupport({
  projects = [],
  endpoint = null,
  conversations = [],
  liveConnect = false,
  ...
  const hasConversations =
    (Array.isArray(conversations) && conversations.length > 0) || Boolean(liveConnect);
  return {
    listProjects: projects.length > 0,
    dynamicEndpoint: Boolean(endpoint?.url),
    listConversations: hasConversations,
    getConversation: hasConversations,
    liveConnect: Boolean(liveConnect),
    // StartCascade is confirmed; SendUserCascadeMessage still model-gated.
    createConversation: Boolean(experimentalWrite),
    dispatchPrompt: Boolean(experimentalWrite),
    experimentalWrite: Boolean(experimentalWrite),
    subscribeEvents: Boolean(liveConnect),
