  ...
  return {
    ...
    async createConversation(projectId, options = {}) {
      ...
      const requestedModel =
        options.requestedModel || buildRequestedModelAlias(options.modelAlias || "AUTO");
      ...
      return {
        ...
        note: "StartCascade succeeded; SendUserCascadeMessage still requires valid model config",
      };
    },
    async dispatchPrompt({ conversationId, prompt, controllerPeerId, modelAlias = "AUTO", cascadeConfig = null } = {}) {
      if (!attached) throw new RemoteSessionError("host_backend_unavailable", "not attached");
      ...
      const requestedModel = buildRequestedModelAlias(modelAlias);
      ...
      // Best-effort inspect after dispatch. Full success depends on model config acceptance.
      let snapshot = null;
      try {
      ...
      const errorEvent = events.find((event) => /error/i.test(event.hostType || event.type || ""));
      if (errorEvent) {
        throw new RemoteSessionError(
          "protocol_error",
