...
function mapStepToEvent(step, index) {
  ...
  if (/WAITING_FOR_INPUT|WAITING_FOR_USER_INPUT|AWAITING_APPROVAL|PENDING_APPROVAL/i.test(stepStatus)) {
    type = "approval_required";
    approvalId = step?.approvalId || step?.id || step?.metadata?.approvalId || `ap_${index + 1}`;
    text = extractAssistantTextFromStep(step) || String(step?.content || step?.text || "Approval required");
  } else if (/USER_INPUT/i.test(typeName) || /USER_EXPLICIT/i.test(source)) {
    type = "user_text";
  ...
  const plannerConfig = {
    ...
    declarativeMixinConfig: {
      promptSections: [
        ...
        { builtinName: "user_information" },
        { builtinName: "mcp_servers" },
        { builtinName: "user_rules" },
        { builtinName: "skills" },
      ],
    },
    plannerTypeConfig: {
...
export async function* pollTrajectoryEvents({
  ...
  while (true) {
    ...
    const nextDelay = idle ? Math.max(200, Number(intervalMs) || 800) : Math.min(250, Number(intervalMs) || 250);
    await sleep(Math.max(50, nextDelay));
  }
}
...
export function summarizeTrajectoryDetail(payload = {}, { cascadeId = "" } = {}) {
  ...
  const pendingApprovals = events
    .filter((event) => event.type === "approval_required" || event.approvalId)
    .map((event) => ({
      approvalId: event.approvalId || `ap_${event.seq}`,
      summary: event.text || "Approval required",
      status: "pending",
      seq: event.seq,
    }));
  return {
