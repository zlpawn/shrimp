...
export function createSshHostBackend({
  ...
  return {
    ...
    async dispatchPrompt({
      ...
      const effectiveCascadeConfig = cascadeConfig || buildCascadeConfig({
      ...
      const script = [
        ...
        `    const postData = JSON.stringify({
          ...
          cascadeConfig: ${JSON.stringify(effectiveCascadeConfig)}
        });`,
    ...
    async subscribeEvents({
      ...
      const script = [
        ...
        '      const localMatches = [...text.matchAll(/Local:\\s+(https:\\/\\/127\\.0\\.0\\.1:(\\d+)\\/?)/g)];',
        '      const csrfMatches = [...text.matchAll(/--csrf_token\\s+([A-Za-z0-9_-]+)/gi)];',
        '      if (localMatches.length && csrfMatches.length) {',
        '        const port = Number(localMatches.at(-1)[1]);',
        '        const csrf = csrfMatches.at(-1)[1];',
        '        if (port && csrf) return { port, csrf };',
        '      }',
        '    } catch {}',
        '  }',
        '  try {',
        '    const psOut = execSync("ps ax -o pid=,command=", { timeout: 3000 }).toString("utf8");',
        '    const serverLine = psOut.split(/\\r?\\n/).find((line) => /language_server/.test(line) && !/grep/.test(line));',
        '    if (serverLine) {',
        '      const pid = Number((serverLine.match(/^\\s*(\\d+)/) || [])[1]);',
        '      const csrf = (serverLine.match(/--csrf_token[= ]([^\\s]+)/) || [])[1];',
        '      const lsof = execSync("lsof -nP -a -p " + pid + " -iTCP -sTCP:LISTEN", { timeout: 3000 }).toString("utf8");',
        '      const ports = [...new Set([...lsof.matchAll(/127\\.0\\.0\\.1:(\\d+)\\s+\\(LISTEN\\)/g)].map((item) => Number(item[1])).filter(Boolean))];',
        '      if (csrf && ports.length > 0) return { port: ports[0], csrf };',
        '    }',
        '  } catch {}',
        '  return null;',
        '}',
        `const srv = findAntigravityServer();`,
        `if (!srv) { console.log(JSON.stringify({ error: "Antigravity language server not found" })); process.exit(0); }`,
        `const postData = JSON.stringify({ cascadeId: "${conversationId}" });`,
        'const req = https.request({',
        '  hostname: "127.0.0.1",',
        '  port: srv.port,',
        '  path: "/exa.language_server_pb.LanguageServerService/GetCascadeTrajectory",',
