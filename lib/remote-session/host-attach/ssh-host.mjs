  ...
  return {
    ...
    async listProjects() {
      const script = [
        ...
        '  } catch {}',
        '}',
        'const projRoot = path.join(home, "project");',
        'if (fs.existsSync(projRoot)) {',
        '  projectsMap.set(projRoot, {',
        '    id: projRoot,',
        ...
        '          let matchedProj = projRoot;',
        '          for (const [pDir, pObj] of projectsMap.entries()) {',
        '            if (pDir !== projRoot && content.includes(pObj.name)) {',
        '              matchedProj = pDir;',
        '              break;',
        '            }',
        '          }',
        ...
        '          };',
        '          if (projectsMap.has(matchedProj)) {',
        '            projectsMap.get(matchedProj).conversations.push(convItem);',
        '          } else if (projectsMap.has(projRoot)) {',
        '            projectsMap.get(projRoot).conversations.push(convItem);',
        '          }',
        '        } catch {}',
        '      }',
        '    }',
    ...
    async getConversation(conversationId) {
      ...
      const script = [
        ...
        '  if (!obj) continue;',
        '  const type = obj.type || "";',
        '  const cText = obj.content || "";',
        '  const toolCalls = (obj.tool_calls || []).map(tc => ({',
        '    name: tc.name || tc.tool || "tool",',
        '    summary: tc.toolSummary || tc.summary || (tc.parameters ? JSON.stringify(tc.parameters).slice(0, 100) : ""),',
        '    status: tc.status || "done"',
        '  }));',
        '  if (type === "USER_INPUT" || cText.includes("<USER_REQUEST>")) {',
        '    let uText = cText;',
        '    const m = cText.match(/<USER_REQUEST>\\s*([\\s\\S]*?)\\s*<\\/USER_REQUEST>/);',
        '    if (m) uText = m[1].trim();',
        '    else uText = uText.replace(/<[^>]+>/g, "").trim();',
        '    if (!title) title = uText.slice(0, 50);',
        '    messages.push({',
        '      id: "msg-" + (obj.step_index ?? messages.length),',
        '      role: "user",',
        '      content: uText,',
        '      timestamp: obj.timestamp || ""',
        '    });',
        '    events.push({ type: "user_message", text: uText, seq: events.length + 1 });',
        '  } else if (type === "PLANNER_RESPONSE" || type === "ASSISTANT" || toolCalls.length > 0 || (obj.source === "MODEL" && cText.trim())) {',
        '    let aText = cText;',
        '    aText = aText.replace(/<think>[\\s\\S]*?<\\/think>/g, "").trim();',
        '    if (aText || toolCalls.length > 0) {',
        '      messages.push({',
        '        id: "msg-" + (obj.step_index ?? messages.length),',
        '        role: "assistant",',
        '        content: aText,',
    ...
    async listAvailableModels() {
      const script = [
        ...
        '  try {',
        '    const ps = execSync("ps aux | grep -i language_server | grep -v grep").toString("utf8");',
        '    const mPid = ps.match(/\\S+\\s+(\\d+)/);',
        '    const mCsrf = ps.match(/--csrf_token\\s+([a-zA-Z0-9_-]+)/);',
        '    if (mPid && mCsrf) {',
        '      const pid = mPid[1];',
        '      const csrf = mCsrf[1];',
        '      const lsof = execSync("lsof -Pan -p " + pid + " -i").toString("utf8");',
    ...
    async createConversation(projectId, { conversationId, model, cascadeConfig } = {}) {
      ...
      const script = [
        ...
        '        const port = Number(mPort[1]);',
        '        const agent = new https.Agent({ rejectUnauthorized: false });',
        `        const postData = JSON.stringify({ workspaceUri: "file://${projectId}" });`,
        '        const resData = await new Promise((resolve) => {',
        '          const req = https.request({',
        '            hostname: "127.0.0.1",',
        '            port,',
    ...
    async dispatchPrompt({
      ...
      const script = [
        ...
        '          req.end();',
        '        });',
        '        return { ok: true, status: resData.status || 200 };',
        '      }',
        '    }',
        '  } catch (e) {',
      ...
      try {
        await runRemoteNodeScript(script, { timeoutMs: 15000 });
      } catch (err) {
