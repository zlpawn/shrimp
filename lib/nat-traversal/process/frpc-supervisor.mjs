 ...
 * - Logs append to a single log file (usually next to frpc.toml)
 */

export function isCrossPlatformMismatch(filePath, platform = process.platform) {
  const p = String(filePath || "").trim();
  if (!p) return false;
  if (platform !== "win32") {
    return /^[a-zA-Z]:[\\/]/.test(p) || /^[a-zA-Z]:$/.test(p) || p.includes("\\");
  }
  return /^\/(Users|home|root|etc|opt|var|tmp|private)\b/i.test(p);
}
...
export function discoverRunningFrpcProcesses({
  ...
  const matches = [];
  const hasMismatch = isCrossPlatformMismatch(configPath, platform);
  ...
  for (const row of rows) {
    ...
    const cmd = String(row.command || row.name || "");
    const isFrpc = name.includes("frpc") || looksLikeFrpcCommand(cmd) || looksLikeFrpcCommand(name);
    if (!isFrpc) continue;

    const cmdNorm = normalizePathForMatch(cmd);
    const extractedConfig = extractConfigPathFromCommand(cmd);
    const extractedBin = extractBinPathFromCommand(cmd);

    let score = 10;
    const configMatched = Boolean(wantedConfig && cmdNorm.includes(wantedConfig));
    if (configMatched) {
      score += 100;
    } else if (wantedConfig && extractedConfig) {
      const wantedBase = path.basename(wantedConfig).toLowerCase();
      const extractedBase = path.basename(extractedConfig).toLowerCase();
      if (wantedBase && wantedBase === extractedBase) {
        score += 30;
      }
    }
    ...
    if (extractedConfig) {
      try {
        if (fs.existsSync(extractedConfig)) score += 20;
      } catch {
        // ignore
      }
    }
    ...
    matches.push({
      ...
      configMatched,
      managed: false,
      binPath: extractedBin || binPath || "",
      configPath: extractedConfig || (configMatched ? configPath : "") || "",
      score,
    });
  }

  const exactMatches = matches.filter((m) => m.configMatched);
  if (exactMatches.length > 0) {
    exactMatches.sort((a, b) => b.score - a.score || a.pid - b.pid);
    return exactMatches;
  }

  // If no config was specified, or if the specified config has a cross-platform mismatch
  if (!wantedConfig || hasMismatch) {
    matches.sort((a, b) => b.score - a.score || a.pid - b.pid);
    return matches;
  }
...
function defaultListProcessCommandLines({ platform = process.platform } = {}) {
  if (platform === "win32") {
    // Filter to frp processes to avoid scanning every process on the machine
    const ps = spawnSync(
      "powershell",
      [
        "-NoProfile",
        "-Command",
        "Get-CimInstance Win32_Process -Filter \"Name like '%frp%'\" | Select-Object ProcessId,Name,CommandLine | ConvertTo-Csv -NoTypeInformation",
      ],
    ...
    // Fallback: WMIC with filter
    const res = spawnSync(
      "wmic",
      ["process", "where", "name like '%frp%'", "get", "ProcessId,Name,CommandLine", "/FORMAT:CSV"],
  ...
  for (const line of String(res.stdout || "").split(/\r?\n/)) {
    ...
    if (!m) continue;
    out.push({ pid: Number(m[1]), command: m[2], name: m[2].split(/\s+/)[0] });
  }
  return out;
}
...
function parseCsvPidCommand(text) {
  ...
  const heads = splitCsvLine(headerLine).map((h) => h.replace(/^"|"$/g, "").toLowerCase());
  const pidIdx = heads.findIndex((h) => h === "processid" || h.endsWith("processid"));
  const nameIdx = heads.findIndex((h) => h === "name" || h.endsWith("name"));
  ...
  for (const line of lines.slice(1)) {
    ...
    if (pidIdx >= 0) {
      ...
      if (nameIdx >= 0) name = String(cols[nameIdx] || "").replace(/^"|"$/g, "");
      if (cmdIdx >= 0) command = String(cols[cmdIdx] || "").replace(/^"|"$/g, "");
    } else if (cols.length >= 2) {
      pid = Number(String(cols[cols.length - 1] || "").replace(/^"|"$/g, ""));
      command = String(cols[cols.length - 2] || "").replace(/^"|"$/g, "");
    }
    if (pid > 0) out.push({ pid, name, command: command || name });
  }
  return out;
}
...
function extractBinPathFromCommand(command) {
  const cmd = String(command || "").trim();
  if (!cmd) return "";
  const m = cmd.match(/^(?:"([^"]+)"|'([^']+)'|([^\s]+))/);
  if (!m) return "";
...
export function createFrpcSupervisor({
  ...
  let cachedAdoptedResult = undefined;
  ...
  function isPidAlive(pid) {
    if (typeof isPidAliveImpl === "function") {
      return Boolean(isPidAliveImpl(pid));
    }
    if (!pid) return false;
    try {
      process.kill(pid, 0);
      ...
      // On Windows, killing an external process with signal 0 throws EPERM when the process is alive.
  ...
  async function start() {
    ...
    let child;
    try {
      child = spawnImpl(binPath, ["-c", configPath], {
        detached: true,
  ...
  function adoptExternalIfPresent() {
    ...
    const now = Date.now();
    if (cachedAdoptedResult !== undefined && now - lastDiscoveryTime < DISCOVERY_CACHE_TTL_MS) {
      if (!cachedAdoptedResult || isPidAlive(cachedAdoptedResult.pid)) {
        return cachedAdoptedResult;
      }
    }
    ...
    cachedAdoptedResult = {
      ...
      command: hit.command,
      binPath: hit.binPath || binPath,
      configPath: hit.configPath || configPath,
    };
    return cachedAdoptedResult;
  }
  ...
  function getStatus() {
    ...
    if (adopted?.configPath) {
      try {
        if (fs.existsSync(adopted.configPath) || !configPath || !fs.existsSync(configPath)) {
          effectiveConfigPath = adopted.configPath;
        }
      } catch {
        // ignore
      }
    ...
    if (adopted?.binPath) {
      try {
        if (fs.existsSync(adopted.binPath) || !binPath || !fs.existsSync(binPath)) {
          effectiveBinPath = adopted.binPath;
        }
      } catch {
        // ignore
      }
    ...
    if (effectiveConfigPath) {
      try {
        const siblingLog = path.join(path.dirname(effectiveConfigPath), "frpc.log");
        if (fs.existsSync(siblingLog)) {
          effectiveLogPath = siblingLog;
        }
      } catch {
        // ignore
      }
    ...
    try {
      if (fs.existsSync(effectiveLogPath)) {
        const content = fs.readFileSync(effectiveLogPath, "utf8");
        const lines = content.split(/\r?\n/).filter(Boolean);
        const tail = lines.slice(-LOG_TAIL_LIMIT);
        recentLines.length = 0;
        recentLines.push(...tail);
      } else if (logPath !== effectiveLogPath && fs.existsSync(logPath)) {
