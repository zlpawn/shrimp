...
export function discoverRunningFrpcProcesses({
  ...
  for (const row of rows) {
    ...
    const configMatched = wantedConfig ? cmdNorm.includes(wantedConfig) : true;
    ...
    matches.push({
      ...
      configMatched,
      managed: false,
      configPath: extractConfigPathFromCommand(cmd) || (configMatched ? configPath : ""),
    });
  }

  // If there are matches with exact config matched, return those
  if (wantedConfig) {
    const exact = matches.filter((m) => m.configMatched);
    if (exact.length > 0) {
      exact.sort((a, b) => a.pid - b.pid);
      return exact;
    }
  }
