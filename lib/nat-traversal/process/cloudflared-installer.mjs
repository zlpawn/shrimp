...
export async function installCloudflared({
  ...
  // If no custom spawnRunner is provided, check if cloudflared is already installed and runnable
  if (!spawnRunner) {
    try {
      const existingBin = resolveCloudflaredBin({ whichBin, platform });
      if (existingBin && fs.existsSync(existingBin)) {
        const testRes = spawnSync(existingBin, ["--version"], {
          encoding: "utf8",
          timeout: 5000,
          windowsHide: true,
        });
        if (!testRes.error && testRes.status === 0) {
          const versionLine = (testRes.stdout || "").trim().split(/\r?\n/)[0];
          logger.info?.(`[cloudflared-installer] cloudflared is already installed: ${existingBin} (${versionLine})`);
          return {
            ok: true,
            message: `cloudflared is already installed (${versionLine})`,
            output: versionLine,
            binPath: existingBin,
          };
        }
  ...
  if (pm === "brew") {
    ...
    args = ["install", "-g", "--allow-scripts=cloudflared", "cloudflared"];
  }
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      child.on("close", (code) => {
        ...
        let binPath = resolveCloudflaredBin({ whichBin, platform });
        if (code === 0) {
          if (platform === "win32" && (!binPath || !fs.existsSync(binPath) || binPath.toLowerCase().endsWith(".cmd"))) {
            try {
              const nodeBinDir = path.dirname(process.execPath);
              const postinstallJs = path.join(nodeBinDir, "node_modules", "cloudflared", "lib", "cloudflared.js");
              if (fs.existsSync(postinstallJs)) {
                logger.info?.("[cloudflared-installer] Running postinstall directly to ensure native binary is present...");
                spawnSync(process.execPath, [postinstallJs, "bin", "install"], {
                  encoding: "utf8",
                  timeout: 120000,
                  windowsHide: true,
                });
                binPath = resolveCloudflaredBin({ whichBin, platform });
              }
            } catch (err) {
              logger.warn?.("[cloudflared-installer] Direct postinstall execution failed:", err);
            }
          }
          done({
            ok: true,
