...
export function createNatTraversalService({
  ...
  function resolveActiveFrpcConfigPath(cfg) {
    ...
    try {
      ...
      if (candidates.length > 0 && candidates[0]?.path && fs.existsSync(candidates[0].path)) {
  ...
  function resolveProviderToken(cfg = getConfig({ reloadFromDisk: false })) {
    if (cfg?.activeProvider === "cloudflared") {
      return secrets.load().cloudflared?.token || "";
    }
    return resolveFrpcToken(cfg);
  }
  ...
  async function status() {
    ...
    if (cfg.activeProvider) {
      providerSummaries[cfg.activeProvider] = providerStatus;
    }
    const otherProviders = providers.list().filter((p) => p.id !== cfg.activeProvider);
    await Promise.all(
      otherProviders.map(async (p) => {
        try {
          const prov = providers.get(p.id);
          if (typeof prov?.status === "function") {
            providerSummaries[p.id] = await prov.status(cfg);
          }
        } catch (e) {
          providerSummaries[p.id] = { status: "error", lastError: e.message || String(e) };
        }
      }),
    );
  ...
  async function start() {
    const cfg = getConfig();
    const provider = activeProvider(cfg);
    const token = resolveProviderToken(cfg);
  ...
  async function openService(peerId, serviceName) {
    ...
    if (!serviceName) {
      throw new NatTraversalError("invalid_request", "service is required");
    }
  ...
  async function fetchDashboardProxies() {
    return dash.fetchProxies();
  }

  async function installCloudflared(options = {}) {
    const provider = providers.get("cloudflared");
    if (!provider || typeof provider.install !== "function") {
      throw new NatTraversalError("unsupported_operation", "cloudflared provider does not support installation");
    }
    return provider.install(options);
  }
