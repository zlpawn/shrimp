...
export async function routeNatTraversalRequest(req, res, _context, reqPath, { service }) {
  try {
    ...
    if (pathOnly === "/v1/nat-traversal/config" && method === "PUT") {
      ...
      const { secrets: incomingSecrets, ...configPatch } = body || {};
      const secrets = incomingSecrets ? { ...incomingSecrets } : {};
      if (body?.cloudflaredToken !== undefined && !secrets.cloudflared) {
        secrets.cloudflared = { token: body.cloudflaredToken };
      }
      if (body?.frpcToken !== undefined && !secrets.frpc) {
        secrets.frpc = { token: body.frpcToken };
      }
    ...
    if (pathOnly === "/v1/nat-traversal/install-cloudflared" && method === "POST") {
      const body = await readJson(req).catch(() => ({}));
      const result = await service.installCloudflared(body || {});
