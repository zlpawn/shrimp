...
export function normalizeNatTraversalConfig(input = {}) {
  ...
  const cfIn = isObject(src.cloudflared) ? src.cloudflared : {};
  ...
  return {
    ...
    cloudflared: normalizeCloudflared(cfIn),
    frpsDashboard: {
...
function normalizeCloudflared(cloudflared) {
  const src = isObject(cloudflared) ? cloudflared : {};
  const mode = asString(src.mode, "quick").trim().toLowerCase();
  return {
    binPath: asString(src.binPath, "").trim(),
    mode: mode === "token" ? "token" : "quick",
    localUrl: asString(src.localUrl, "http://127.0.0.1:8787").trim() || "http://127.0.0.1:8787",
    logLevel: asString(src.logLevel, "info").trim() || "info",
    publicUrl: asString(src.publicUrl, "").trim(),
  };
}
...
export function publicConfigView(config, secretsMeta = {}) {
  ...
  return {
    ...
    secrets: {
      ...
      cloudflaredTokenConfigured: Boolean(secretsMeta.cloudflaredTokenConfigured),
