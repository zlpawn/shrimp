...
import { installCloudflared } from "../process/cloudflared-installer.mjs";
...
export function createCloudflaredProvider({
  ...
  function ensureSupervisor(config) {
    ...
    if (!supervisor || supervisor.binPath !== binPath || supervisor.pidPath !== pidPath || supervisor.logPath !== logPath) {
      ...
      supervisor.binPath = binPath;
      supervisor.pidPath = pidPath;
      supervisor.logPath = logPath;
    }
  ...
  return {
    ...
    async install({ packageManager = "npm", timeoutMs = 180000 } = {}) {
      return installCloudflared({
        packageManager,
        timeoutMs,
        logger,
        spawnRunner: installRunner || cloudflaredRunner?.installRunner,
        whichBin,
      });
    },
  };
