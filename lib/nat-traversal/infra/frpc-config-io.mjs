...
function looksLikeFrpDirName(name) {
  ...
  // frp_0.71.0 / frp_0.71.0_darwin_arm64 / frp-0.69.1-darwin-arm64
...
function collectFrpcFilesInDir(dirPath, out, depth = 0) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory() && !entry.name.startsWith(".") && depth < 2) {
      // one more level for extracted archives
      if (
        looksLikeFrpDirName(entry.name) ||
        entry.name.toLowerCase() === "conf" ||
        entry.name.toLowerCase() === "config"
      ) {
...
export function listFrpcCandidatePaths({
  ...
  // Also scan Downloads/Desktop shallowly for extracted frp archives.
  for (const base of [
