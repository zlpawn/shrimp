...
export function scanInRepoClis(configDir = process.cwd(), fsImpl = fs) {
  ...
  // 1. Scan lib/skills for self-contained Skill CLIs (package.json with bin or scripts)
  ...
  for (const entry of skillEntries) {
    const isDir = typeof entry === "string"
      ? fsImpl.statSync(path.join(skillsDir, entry)).isDirectory()
      : entry.isDirectory();
    if (!isDir) continue;
    ...
    if (!fsImpl.existsSync(pkgFile)) continue;
    ...
    try {
      ...
      const binMap = typeof pkg.bin === "string" ? { [subName]: pkg.bin } : (pkg.bin || {});
      for (const [binName, binRel] of Object.entries(binMap)) {
        const cleanBinRel = String(binRel).replace(/^\.\//, "");
        const targetPath = path.join(skillSubDir, cleanBinRel);
        if (!fsImpl.existsSync(targetPath)) continue;

        const relPath = `./lib/skills/${subName}/${cleanBinRel}`.replaceAll("\\", "/");
        const isPy = cleanBinRel.endsWith(".py");
        const lang = isPy ? "python" : "node";
        const langLabel = isPy ? "Python (uv)" : "Node.js";
        const command = isPy ? "uv" : "node";
        const args = isPy ? ["run", "--directory", `./lib/skills/${subName}`, cleanBinRel] : [relPath];
        const fullCommand = isPy ? `uv run --directory ./lib/skills/${subName} ${cleanBinRel}` : `node ${relPath}`;

        seenNames.add(binName);
        list.push({
          name: binName,
          lang,
          langLabel,
          title: `${binName} (${langLabel} CLI)`,
          description: skillDesc || `位于 ${relPath} 的自研 ${langLabel} 命令行工具`,
          command,
          args,
          fullCommand,
          path: relPath,
  ...
  // 2. Scan clis/ directory for standalone custom in-repo CLIs (backward compatibility)
  const clisDir = path.join(configDir, "clis");
  try {
    if (!fsImpl.existsSync(clisDir)) return list;
  } catch {
    return list;
  }
  let entries;
