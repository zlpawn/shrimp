...
async function startGateway(context) {
  ...
  if (existing && isProcessRunning(existing.pid)) {
    ...
    const listeningPid = await findListeningPid(existing.port || port);
    if (listeningPid !== existing.pid) {
...
async function stopGateway(context) {
  ...
  if (!matches) {
    const listeningPid = await findListeningPid(metadata.port);
    if (listeningPid !== metadata.pid) {
      await removeFile(paths.pidFile);
      await removeFile(paths.legacyPidFile);
      io.log(`Gateway is not running (stale PID ${metadata.pid} was recycled by another process).`);
      return;
    }
    if (!force) {
      throw new Error(
