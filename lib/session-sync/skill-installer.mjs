...
const DEFAULT_SKILLS_CONFIG_FILE = process.env.SKILLS_CONFIG_FILE || path.join(PROJECT_ROOT, "skills.config.json");
...
export class SkillInstaller {
  ...
  static getToolPathMeta(skillName = "session-sync", customClients = [], homeDir = os.homedir()) {
    const customConfig = SkillInstaller.loadSkillsConfig();
    const customPaths = customConfig?.clientPaths || {};
    ...
    for (const client of (customClients || [])) {
      const skillDir = SkillInstaller.resolveCustomSkillDir(client, homeDir, customPaths);
      const relativeOrAbs = skillDir.startsWith(homeDir)
        ? `~${skillDir.slice(homeDir.length)}`
        : skillDir;
      base[client] = {
        label: client,
        short: client.slice(0, 1).toUpperCase(),
        color: "#64748b",
        pathTemplate: path.join(relativeOrAbs, "{name}"),
        path: path.join(relativeOrAbs, skillName),
        isCentral: false,
        isCustom: true,
      };
    }
  ...
  static buildLibrarySnapshot({
    ...
    const skills = SkillInstaller.listCentralSkills(homeDir, customClients).map((skill) => {
      const tools = SkillInstaller.getToolPathMeta(skill.name, customClients, homeDir);
      return {
        ...skill,
