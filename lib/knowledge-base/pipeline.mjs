...
export function createKbPipeline({ store, registry, scraper, dataDir }) {
  ...
  async function reparseDocument(id, { engine = "markitdown" } = {}) {
    const existing = store.getDocument(id);
    if (!existing) return null;
    if (engine !== "markitdown" && engine !== "docling") {
      throw new Error("Unsupported engine, expected markitdown or docling");
    }
    if (existing.source_type !== "file" || !existing.file_path) {
      throw new Error("Only documents with a retained local source file can be reparsed");
    }
    if (!fs.existsSync(existing.file_path)) {
      throw new Error("Original source file no longer exists");
    }

    const adapter = registry.get(engine);
    if (!adapter) throw new Error(`Parser engine unavailable: ${engine}`);
    const status = await adapter.detect();
    if (!status.installed) throw new Error(`Parser engine not installed: ${engine}`);

    store.updateDocumentResult(id, {
      [`${engine}_status`]: "running",
      [`${engine}_md`]: "",
      [`${engine}_html`]: "",
      [`${engine}_duration_ms`]: 0,
      [`${engine}_error`]: "",
    });
    ...
    try {
      const outputDir = path.join(
        filesDir,
        id,
        engine === "docling" ? "docling_out" : "markitdown_out"
      );
      const assetsDir = path.join(filesDir, id, "assets");
      fs.mkdirSync(outputDir, { recursive: true });
      fs.mkdirSync(assetsDir, { recursive: true });
      const result = await adapter.parse(existing.file_path, {
        outputDir,
        assetsDir,
      });
      if (result.markdown) {
        fs.mkdirSync(path.join(filesDir, id), { recursive: true });
        fs.writeFileSync(path.join(filesDir, id, `${engine}.md`), result.markdown, "utf8");
      }
      const current = store.getDocument(id);
      const updates = {
        [`${engine}_status`]: "done",
        [`${engine}_md`]: result.markdown || "",
        [`${engine}_html`]: result.html || null,
        [`${engine}_duration_ms`]: result.durationMs || 0,
        [`${engine}_error`]: "",
      };
      if (engine === "docling") {
        updates.docling_json = result.jsonStructure ? JSON.stringify(result.jsonStructure) : "";
        updates.assets = result.assets?.length ? result.assets : current.assets;
      }
      const candidateMd = engine === "markitdown" ? result.markdown : current.docling_md;
      const otherMd = engine === "markitdown" ? current.docling_md : result.markdown;
      updates.word_count = Math.max(
        String(candidateMd || "").replace(/\s+/g, "").length,
        String(otherMd || "").replace(/\s+/g, "").length
      );
      return store.updateDocumentResult(id, updates);
    } catch (err) {
      return store.updateDocumentResult(id, {
        [`${engine}_status`]: "failed",
        [`${engine}_md`]: "",
        [`${engine}_html`]: null,
        [`${engine}_duration_ms`]: 0,
        [`${engine}_error`]: err.message,
      });
    }
  }
