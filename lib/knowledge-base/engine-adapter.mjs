...
import { detectWhisperTools } from "../video-kb/transcriber.mjs";
...
export class WhisperAdapter extends BaseEngineAdapter {
  constructor() {
    super({
      id: "whisper",
      name: "Whisper",
      description: "音视频逐句语音识别转录，支持精确时间戳切片对齐与多语种识别",
      command: "whisper-ctranslate2",
      supportedExtensions: [".mp4", ".mp3", ".wav", ".m4a", ".webm", ".mkv", ".flv"],
    });
  }
  ...
  async detect() {
    try {
      ...
      if (tools.length > 0) {
        ...
        const v = primary.version ? primary.version.replace(/^whisper-ctranslate2\s*/i, "") : "已就绪";
        return {
          id: this.id,
          name: this.name,
          installed: true,
          version: v,
          binPath: primary.path,
          platformSupport: true,
          installCommand: primary.install || "uv tool install whisper-ctranslate2",
          upgradeCommand: "uv tool upgrade whisper-ctranslate2",
          activeEngine: primary.name || primary.id,
        };
      }
    } catch { /* ignore */ }

    const baseStatus = await super.detect();
    return {
      ...baseStatus,
      installCommand: "uv tool install whisper-ctranslate2",
      upgradeCommand: "uv tool upgrade whisper-ctranslate2",
    };
  }

  async install(onLog) {
    return await runProcess("uv", ["tool", "install", "whisper-ctranslate2"], { onLog });
  }

  async upgrade(onLog) {
    return await runProcess("uv", ["tool", "upgrade", "whisper-ctranslate2"], { onLog });
  }
}
