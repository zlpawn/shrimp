...
import { selectEndpointCredential } from "./key-strategy.mjs";
  ...
  for (const [clientName, client] of Object.entries(config?.clients || {})) {
    ...
    for (const [index, endpoint] of endpoints.entries()) {
      ...
      if (Array.isArray(endpoint.api_keys)) {
        if (endpoint.api_keys.length === 0) {
          issues.push({
            ...
            client: clientName,
            endpoint_id: id,
            message: `Client '${clientName}' endpoint '${id}' must declare at least one credential.`,
          });
        }
        ...
        for (const credential of endpoint.api_keys) {
          ...
          if (!credentialId) {
            issues.push({
              code: "empty_credential_id",
              client: clientName,
              endpoint_id: id,
              message: `Client '${clientName}' endpoint '${id}' has an empty credential id.`,
            });
            continue;
          }
          if (credentialIds.has(credentialId)) {
            issues.push({
              ...
              message: `Client '${clientName}' endpoint '${id}' repeats credential '${credentialId}'.`,
            });
          }
      ...
      if (
        ...
        && !["failover", "round-robin", "random"].includes(
          endpoint.key_strategy,
        )
      ) {
        issues.push({
          code: "invalid_key_strategy",
          client: clientName,
          endpoint_id: id,
          message: `Client '${clientName}' endpoint '${id}' has an unsupported key strategy.`,
        });
      }
