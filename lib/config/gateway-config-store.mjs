...
// the built-in pool, then falls back to incrementing version numbers via
// nextClaudeVersionSuggestion when the pool is exhausted.
export function allocateClaudePublicId(usedIds = []) {
  ...
  // Pool exhausted: derive the next available versioned name from the last id.
