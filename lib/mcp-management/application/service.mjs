...
import { listClientAdapters, getClientAdapter } from "../clients/registry.mjs";
...
export function resolveCustomMcpPath(client, config = {}, homeDir = os.homedir(), fsImpl = fs) {
  ...
  const candidates = [
    path.join(homeDir, `.${stripped}`, "mcp.json"),
    path.join(homeDir, `.${raw}`, "mcp.json"),
    path.join(homeDir, `.${stripped}`, ".mcp.json"),
    path.join(homeDir, `.${raw}`, ".mcp.json"),
  ];
  ...
  for (const candidate of candidates) {
    try {
      if (fileExists(fsImpl, candidate) || (fsImpl.existsSync && fsImpl.existsSync(candidate))) {
        return candidate;
      }
    } catch {
      // ignore
  ...
  return path.join(homeDir, `.${stripped}`, "mcp.json");
}
...
export function createMcpManagementService({
  ...
  const effectiveHome = homeDir || home;
  ...
  function requireAdapter(client) {
    const adapter = getClientAdapter(String(client || ""));
    if (!adapter) throw new McpManagementError("client_not_found", "Unknown client: " + client);
    return adapter;
  }

  function resolveClientPath(client, config) {
    const clientId = String(client || "").trim();
    const override = String(config?.clientPaths?.[clientId] || "").trim();
    if (override) {
      return override.startsWith("~/") || override.startsWith("~\\")
        ? path.join(effectiveHome, override.slice(2))
        : path.resolve(override);
    }
    if (BUILTIN_CLIENT_IDS.includes(clientId)) {
      return requireAdapter(clientId).defaultPath(effectiveHome, platform);
    }
    return resolveCustomMcpPath(clientId, config, effectiveHome, fsImpl);
  }

  function resolvedClientPaths(config, customClientIds = []) {
    const allIds = resolveAllClientIds([
      ...(customClientIds || []),
      ...Object.keys(config?.clientPaths || {}),
    ]);
    const out = {};
    for (const id of allIds) out[id] = resolveClientPath(id, config);
    return out;
  }

  function normalizeTargets(input = {}, customClientIds = []) {
    const allIds = resolveAllClientIds([
      ...(customClientIds || []),
      ...(Array.isArray(input) ? input : Object.keys(input || {})),
    ]);
    if (Array.isArray(input)) {
      const out = {};
      for (const id of allIds) out[id] = input.includes(id);
      return out;
    }
    const src = input && typeof input === "object" ? input : {};
    const out = {};
    for (const id of allIds) out[id] = Boolean(src[id]);
    return out;
  }
  ...
  function scanClients(config, customClientIds = []) {
    const allIds = resolveAllClientIds([
      ...(customClientIds || []),
      ...Object.keys(config?.clientPaths || {}),
    ]);
    const clients = allIds.map((client) => scanClient(client, config));
    const presentIn = {};
  ...
  function scanInRepoMcps(configDir) {
    ...
    // 1. Scan lib/skills for self-contained Skill MCPs (package.json with mcp or scripts/mcp/index.mjs)
    const skillsDir = path.join(configDir, "lib", "skills");
    let skillEntries = [];
    try {
      if (fsImpl.existsSync(skillsDir)) {
        skillEntries = fsImpl.readdirSync(skillsDir, { withFileTypes: true });
      }
    } catch {}

    for (const entry of skillEntries) {
      const isDir = typeof entry === "string" ? fsImpl.statSync(path.join(skillsDir, entry)).isDirectory() : entry.isDirectory();
      if (!isDir) continue;
      const subName = typeof entry === "string" ? entry : entry.name;
      if (subName.startsWith(".") || subName === "node_modules") continue;

      const skillSubDir = path.join(skillsDir, subName);
      const pkgFile = path.join(skillSubDir, "package.json");
      let pkg = {};
      if (fsImpl.existsSync(pkgFile)) {
        try {
          const raw = fsImpl.readFileSync(pkgFile, "utf8").replace(/^\uFEFF/, "");
          pkg = JSON.parse(raw);
        } catch {}
      }

      let skillDesc = "";
      const skillMdFile = path.join(skillSubDir, "SKILL.md");
      if (fsImpl.existsSync(skillMdFile)) {
        try {
          const content = fsImpl.readFileSync(skillMdFile, "utf8");
          const descMatch = content.match(/^description:\s*(.+)$/m);
          if (descMatch) skillDesc = descMatch[1].trim();
        } catch {}
      }
      if (!skillDesc && pkg.description) skillDesc = pkg.description;

      const mcpMap = typeof pkg.mcp === "string" ? { [subName]: pkg.mcp } : (pkg.mcp || {});
      const mcpEntries = Object.entries(mcpMap);

      if (mcpEntries.length > 0) {
        for (const [mcpName, mcpRel] of mcpEntries) {
          const cleanRel = String(mcpRel).replace(/^\.\//, "");
          const targetPath = path.join(skillSubDir, cleanRel);
          if (!fsImpl.existsSync(targetPath)) continue;

          const relPath = `./lib/skills/${subName}/${cleanRel}`.replaceAll("\\", "/");
          const isDbHub = mcpName === "database-hub";
          ...
          list.push({
            name: mcpName,
            lang: "node",
            title: isDbHub ? "database-hub (多数据库 MCP)" : `${mcpName} (Node.js Skill MCP)`,
            description: skillDesc || (isDbHub
              ? "支持 MySQL / Redis / SQLite 的多数据源连接、NL2SQL 查询与表结构管理 MCP"
              : `位于 ${relPath} 的自研 Node.js MCP 服务`),
            command: "node",
            args: [relPath],
            transport: "stdio",
            path: relPath,
        ...
        const defaultMcpPath = path.join(skillSubDir, "scripts", "mcp", "index.mjs");
        if (fsImpl.existsSync(defaultMcpPath)) {
          const mcpName = subName;
          const relPath = `./lib/skills/${subName}/scripts/mcp/index.mjs`.replaceAll("\\", "/");
          seenNames.add(mcpName);
          list.push({
            name: mcpName,
            lang: "node",
            title: `${mcpName} (Node.js Skill MCP)`,
            description: skillDesc || `位于 ${relPath} 的自研 Node.js MCP 服务`,
            command: "node",
            args: [relPath],
            transport: "stdio",
            path: relPath,
    ...
    // 2. Scan mcps/ directory for backward compatibility
    const mcpsDir = path.join(configDir, "mcps");
    let entries = [];
    try {
      if (fsImpl.existsSync(mcpsDir)) {
    ...
    for (const entry of entries) {
      ...
      if (!isDir) continue;
      const subName = typeof entry === "string" ? entry : entry.name;
      if (subName.startsWith(".") || subName === "node_modules" || seenNames.has(subName)) continue;
  ...
  function state(customClientIds = []) {
    const { config, secrets } = store.load();
    const allIds = resolveAllClientIds([
      ...(customClientIds || []),
      ...Object.keys(config?.clientPaths || {}),
    ]);
    const paths = resolvedClientPaths(config, customClientIds);
    const scanResult = scanClients(config, customClientIds);
    const configDir = store.configPath ? path.dirname(store.configPath) : process.cwd();
    return {
      config,
      ...
      clientsMeta: allIds.map((id) => {
        const adapter = requireAdapter(id);
        const defaultPath = BUILTIN_CLIENT_IDS.includes(id)
          ? adapter.defaultPath(effectiveHome, platform)
          : resolveCustomMcpPath(id, {}, effectiveHome, fsImpl);
        return {
          id,
          label: adapter.label || id,
          path: paths[id],
          defaultPath,
          custom: !BUILTIN_CLIENT_IDS.includes(id),
        };
      }),
    };
  }

  function scan(customClientIds = []) {
    const { config } = store.load();
    return {
      paths: resolvedClientPaths(config, customClientIds),
      ...scanClients(config, customClientIds),
    };
  }
  ...
  function buildPreviews(config, secrets, targets, serverName = null) {
    const previews = [];
    for (const [client, isTarget] of Object.entries(targets)) {
  ...
  function preview({ targets = null, serverName = null, customClientIds = [] } = {}) {
    const { config, secrets } = store.load();
    ...
    const effectiveTargets = (targets && Object.keys(targets).length > 0)
      ? targets
      : (targetName && config.servers[targetName] ? config.servers[targetName].distribution : targets || {});
    const normalizedTargets = normalizeTargets(effectiveTargets, customClientIds);
    return {
  ...
  function apply({ targets = null, serverName = null, customClientIds = [] } = {}) {
    const { config, secrets } = store.load();
    const targetName = serverName ? String(serverName).trim() : null;
    if (targetName && !config.servers[targetName]) {
      throw new McpManagementError("server_not_found", "Unknown server: " + targetName);
    ...
    const planned = [];
    for (const [client, isTarget] of Object.entries(normalizedTargets)) {
      if (!isTarget) continue;
      const adapter = requireAdapter(client);
      const filePath = resolveClientPath(client, config);
  ...
  function setClientPath({ client, path: filePath } = {}, customClientIds = []) {
    const { config } = store.load();
    const clientId = String(client || "").trim();
    if (!clientId) {
      throw new McpManagementError("invalid_request", "client is required");
    }
    const rawPath = String(filePath || "").trim();
    if (rawPath) {
      const expectedExtension = CLIENT_FILE_EXTENSIONS[clientId] || ".json";
      if (path.extname(rawPath).toLowerCase() !== expectedExtension) {
        const adapter = requireAdapter(clientId);
        throw new McpManagementError(
          "invalid_request",
          (adapter ? adapter.label : clientId) + " 配置文件必须是 " + expectedExtension + " 文件",
    ...
    const extra = Array.isArray(customClientIds) ? [...customClientIds] : [];
    if (clientId && !BUILTIN_CLIENT_IDS.includes(clientId) && !extra.includes(clientId)) {
      extra.push(clientId);
    }
    return state(extra);
  }
