...
import { createInspectorProxy } from "../infra/inspector-proxy.mjs";
...
export function createMcpManagementService({
  ...
  inspectorProxy: injectedInspectorProxy,
  logger = console,
} = {}) {
  ...
  const inspectorProxy = injectedInspectorProxy || createInspectorProxy({
    inspectorManager,
    logger,
  });
  ...
  function mergedServer(server, secrets) {
    ...
    const resolvedUrl = interpolateString(server.url, combinedEnv);
  ...
  async function handleInspectorProxy(req, res, serverName, subPath) {
    return await inspectorProxy.handle(req, res, serverName, subPath);
  }
