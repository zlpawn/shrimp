...
export function createMcpManagementService({
  ...
  function resolveClientPath(client, config) {
    ...
    if (BUILTIN_CLIENT_IDS.includes(clientId)) {
      return requireAdapter(clientId).defaultPath(home, platform);
    }
    return resolveCustomMcpPath(clientId, config, home, fsImpl);
  }
    ...
    return {
      ...
      clientsMeta: allIds.map((id) => {
        ...
        const defaultPath = BUILTIN_CLIENT_IDS.includes(id)
          ? adapter.defaultPath(home, platform)
          : resolveCustomMcpPath(id, {}, home, fsImpl);
        return {
          id,
  ...
  function preview({ targets = {}, serverName = null, customClientIds = [] } = {}) {
    const { config, secrets } = store.load();
    const normalizedTargets = normalizeTargets(targets, customClientIds);
  ...
  function apply({ targets = {}, serverName = null, customClientIds = [] } = {}) {
    const normalizedTargets = normalizeTargets(targets, customClientIds);
    const { config, secrets } = store.load();
    const targetName = serverName ? String(serverName).trim() : null;
    if (targetName && !config.servers[targetName]) {
