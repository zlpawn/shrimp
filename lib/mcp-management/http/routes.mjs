...
export async function routeMcpManagementRequest(req, res, _context, reqPath, { service }) {
  try {
    ...
    // Inspector reverse proxy route
    const proxyMatch = pathOnly.match(/^\/v1\/mcp-management\/inspector-proxy\/([^/]+)(\/.*)?$/);
    if (proxyMatch) {
      const serverName = decodeURIComponent(proxyMatch[1]);
      const subPath = proxyMatch[2] || "/";
      return await service.handleInspectorProxy(req, res, serverName, subPath);
    }
