...
import { fetchVideoInfo, downloadVideo, videoIdFromUrl, extractVideoFrames } from "./downloader.mjs";
...
import { transcribeImageWithVisionLLM } from "../knowledge-base/vision-transcriber.mjs";
...
const NODE_DEFS = [
  ...
  },
  {
    ...
    weight: 0.08,
    defaultEnabled: true,
    requires: ["download_video"],
    async run(ctx, { signal, onProgress }) {
      if (ctx.frameStrategy === "none") {
        onProgress?.(1, "用户设置跳过抽帧");
        return { frames: [], frameCount: 0 };
      }
      if (!ctx.videoPath || !fs.existsSync(ctx.videoPath)) {
        onProgress?.(1, "未找到视频素材，跳过抽帧");
        return { frames: [], frameCount: 0 };
      }
      const framesDir = path.join(ctx.outputDir, "frames");
      onProgress?.(0.1, "开始视频抽帧...");
      const frames = await extractVideoFrames(ctx.videoPath, framesDir, {
        strategy: ctx.frameStrategy || "scene",
        interval: Number(ctx.frameInterval) || 5,
        sceneThreshold: Number(ctx.sceneThreshold) || 0.35,
        maxFrames: Number(ctx.maxFrames) || 30,
        signal,
        onProgress: (frac, msg) => onProgress?.(frac, msg),
      });
      onProgress?.(1, `抽帧完成，共提取 ${frames.length} 个候选关键帧`);
      return {
        frames,
  ...
  },
  {
    ...
    weight: 0.12,
    defaultEnabled: false,
    requires: ["extract_frames"],
    async run(ctx, { signal, onProgress }) {
      if (!ctx.frames || ctx.frames.length === 0) {
        onProgress?.(1, "没有待解读的关键帧，跳过");
        return { evidenceCards: [] };
      }
      const limit = Math.min(Number(ctx.maxVisionFrames) || 5, ctx.frames.length);
      const sampleFrames = ctx.frames.slice(0, limit);
      const evidenceCards = [];

      for (let i = 0; i < sampleFrames.length; i++) {
        if (signal?.aborted) throw new Error("Vision audit cancelled");
        const frame = sampleFrames[i];
        onProgress?.(i / limit, `正在解读关键帧 ${i + 1}/${limit} (${frame.timestampFormatted})`);
        try {
          const res = await transcribeImageWithVisionLLM({
            imagePath: frame.path,
            filename: frame.filename,
            sourceType: "video",
            timestampOrPage: frame.timestampFormatted,
            client: ctx.visionClient || ctx.summaryClient || "code",
            endpointId: ctx.visionEndpointId || ctx.summaryEndpointId || "",
            model: ctx.visionModel || "qwen-vl-max",
            listenPort: ctx.listenPort || 8788,
            signal,
          });
          evidenceCards.push({
            ...
            cardHtml: res.cardHtml,
            mermaidCode: res.mermaidCode,
            summaryPoints: res.bulletPoints,
          });
        } catch (err) {
          console.error(`[video-kb] Vision audit frame ${frame.filename} failed:`, err.message);
        }
      }
      onProgress?.(1, `视觉大模型解读完成，生成 ${evidenceCards.length} 张证据卡片`);
      return { evidenceCards };
    },
  },
  {
