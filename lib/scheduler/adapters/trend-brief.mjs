...
export class TrendBriefAdapter extends BaseJobAdapter {
  ...
  getStatus(jobConfig = {}) {
    ...
    const isEnabled = jobConfig.enabled ?? schedulerStatus?.brief_enabled ?? true;
    ...
    return {
      ...
      nextRunAt: isEnabled ? this._calculateNextDailyRun(dailyTimes) : null,
    };
  }
  ...
  async runNow() {
    ...
    const result = await service.generateBriefOnce({ date: todayStr });
  ...
  /**
   * Write-through to the trend-intel config store. The shared minute timer
   * re-reads daily_brief_times on every tick, so no timer restart is needed.
   */
  async onConfigChange(patch, updatedConfig) {
    ...
    if (Array.isArray(updatedConfig.daily_times)) {
      schedulerPatch.daily_brief_times = updatedConfig.daily_times;
    }
    if (typeof updatedConfig.enabled === "boolean") {
      schedulerPatch.brief_enabled = updatedConfig.enabled;
    }
    ...
    // The crawl and daily-brief jobs share one scheduler. If both were
    // disabled, that shared scheduler was stopped; starting either sub-job
    // must bring it back. The minute timer reads the latest config each tick.
    ...
    if (updatedConfig.enabled === false && scheduler?.stop) {
      ...
      if (status && status.crawl_enabled === false) {
        scheduler.stop();
      }
    }
  }
