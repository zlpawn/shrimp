...
export async function routeSessionKanbanRequest(req, res, reqPath, { service } = {}) {
  try {
    ...
    if (pathOnly === "/v1/session-kanban/paths" && method === "GET") {
      return sendJson(res, 200, await service.getPathsConfig());
    }
    if (pathOnly === "/v1/session-kanban/paths" && (method === "PUT" || method === "POST")) {
      const body = await readJson(req);
      return sendJson(res, 200, await service.setPathsConfig(body));
    }
    if (pathOnly === "/v1/session-kanban/paths/reset" && method === "POST") {
      await readJson(req);
      return sendJson(res, 200, await service.resetPathsConfig());
    }
    ...
    const transcriptMatch = pathOnly.match(/^\/v1\/session-kanban\/sessions\/([^/]+)\/transcript$/);
    if (transcriptMatch && method === "GET") {
      const id = decodeURIComponent(transcriptMatch[1]);
      return sendJson(res, 200, await service.getTranscript(id));
    }

    const scheduleMatch = pathOnly.match(/^\/v1\/session-kanban\/queue\/([^/]+)\/schedule$/);
    if (scheduleMatch && (method === "PATCH" || method === "POST" || method === "PUT")) {
      const body = await readJson(req);
      const id = decodeURIComponent(scheduleMatch[1]);
      return sendJson(res, 200, await service.updateSchedule(id, body));
    }
    ...
    if (action && method === "POST") {
      ...
      const result = action[2] === "cancel" ? await service.cancel(id) : await service.retry(id, body);
