    ...
    async dispatchReady() {
      ...
      for (const item of candidateQueue) {
        ...
        if (!session) {
          ...
          const settled = await store.markFailed(item.id, "Target session not found or inactive (>48h)");
          if (typeof onQueueSettled === "function") {
            await onQueueSettled(settled);
          }
          continue;
        }
        ...
        if (!dispatcher) {
          await store.claimForDispatch(item.id);
          const settled = await store.markFailed(item.id, "No supported dispatcher for this session");
          if (typeof onQueueSettled === "function") {
            await onQueueSettled(settled);
          }
          continue;
        ...
        try {
          ...
          const settled = await store.markDispatched(item.id, {
          ...
          if (quota.isQuotaError) {
            ...
            const settled = await store.markFailed(item.id, error?.message || String(error));
            if (typeof onQueueSettled === "function") {
              await onQueueSettled(settled);
            }
          }
