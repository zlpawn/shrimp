...
function publicRow(row) {
  ...
  const notifiedAtMs = Number(row.notified_at_ms || 0);
  return {
    id: row.id,
    ...
    notifyStatus: row.notify_status || "none",
    notifyAttempts: Number(row.notify_attempts || 0),
    notifyError: row.notify_error || "",
    notifiedAtMs,
    notifiedAt: notifiedAtMs ? new Date(notifiedAtMs).toISOString() : null,
...
export function createSessionKanbanStore({ dbPath = "gateway.db" } = {}) {
  ...
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN source TEXT NOT NULL DEFAULT '';"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN binding_key TEXT NOT NULL DEFAULT '';"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN platform TEXT NOT NULL DEFAULT '';"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN chat_type TEXT NOT NULL DEFAULT '';"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN chat_id TEXT NOT NULL DEFAULT '';"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN user_id TEXT NOT NULL DEFAULT '';"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN reply_to_message_id TEXT NOT NULL DEFAULT '';"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN notify_status TEXT NOT NULL DEFAULT 'none';"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN notify_attempts INTEGER NOT NULL DEFAULT 0;"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN notify_error TEXT NOT NULL DEFAULT '';"); } catch {}
  try { db.exec("ALTER TABLE session_kanban_queue ADD COLUMN notified_at_ms INTEGER NOT NULL DEFAULT 0;"); } catch {}
  ...
  const insert = db.prepare("INSERT INTO session_kanban_queue " +
    "(id, session_id, message, status, attempts, error, dispatch_command, dispatch_exit_code, scheduled_at_ms, vendor_tag, " +
    "source, binding_key, platform, chat_type, chat_id, user_id, reply_to_message_id, notify_status, notify_attempts, notify_error, notified_at_ms, " +
    "created_at_ms, updated_at_ms) " +
    "VALUES (:id, :session_id, :message, :status, 0, '', '', NULL, :scheduled_at_ms, :vendor_tag, " +
    ":source, :binding_key, :platform, :chat_type, :chat_id, :user_id, :reply_to_message_id, :notify_status, 0, '', 0, " +
    ":now, :now)");
  ...
  const notifySentUpdate = db.prepare("UPDATE session_kanban_queue " +
    "SET notify_status = 'sent', notify_error = '', notified_at_ms = :now, updated_at_ms = :now " +
    "WHERE id = :id");
  const notifyFailedUpdate = db.prepare("UPDATE session_kanban_queue " +
    "SET notify_status = 'failed', notify_attempts = notify_attempts + 1, notify_error = :error, " +
  ...
  return {
    ...
    enqueue({
      ...
      const notifyStatus = sourceValue === "remote-agent" ? "pending" : "none";
    ...
    markNotifySent(id) {
      return transition(notifySentUpdate, { id, now: Date.now() });
    },
    markNotifyFailed(id, error) {
      return transition(notifyFailedUpdate, {
        id,
        error: String(error || "notify failed").slice(0, 300),
        now: Date.now(),
      });
    },
    getSetting(key) {
