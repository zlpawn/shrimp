...
function syncAntigravitySessionBack(id) {
  try {
    ...
    const cliTranscript = path.join(cliDir, "brain", id, ".system_generated", "logs", "transcript.jsonl");
    const desktopTranscript = path.join(desktopDir, "brain", id, ".system_generated", "logs", "transcript.jsonl");
    if (fs.existsSync(cliTranscript) && fs.existsSync(path.dirname(desktopTranscript))) {
      fs.copyFileSync(cliTranscript, desktopTranscript);
    }
  } catch {}
}
