...
function parseAntigravityTranscript(transcriptPath, sessionId) {
  ...
  for (const line of fs.readFileSync(transcriptPath, "utf8").split("\n")) {
    ...
    try {
      ...
      // Extract tool calls from any step that declares them
      if (Array.isArray(item.tool_calls) && item.tool_calls.length > 0) {
        for (const tc of item.tool_calls) {
          ...
          pendingTools.push({ name, summary: String(summary || "").replace(/^"|"$/g, ""), detail: String(detail || "").replace(/^"|"$/g, "").slice(0, 500) });
        }
      }
      ...
      // Skip GENERIC / CHECKPOINT / TOOL_RESULT dumps
    } catch {}
  }

  // Attach leftover tools if any
    ...
    try {
      ...
      if (item.type === "event_msg") {
        ...
        if (ptype === "user_message" && msg) {
          const text = cleanUserContent(msg);
          if (text) {
            if (!title) title = cleanSessionTitle(text);
            messages.push({
      ...
      // Skip function_call_output raw dumps
    } catch {}
  }
