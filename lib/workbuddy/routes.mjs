...
export async function routeWorkbuddyRequest(req, res, context, reqPath) {
  try {
    ...
    if (req.method === "POST" && (pathname === "/v1/workbuddy/check-legacy" || pathname === "/v1/workbuddy/check-uv")) {
      const result = await checkLegacyCommands({ port: queryPort });
      return sendJson(res, 200, { success: true, ...result });
    }

    if (req.method === "POST" && (pathname === "/v1/workbuddy/cleanup-uv" || pathname === "/v1/workbuddy/cleanup-legacy")) {
      const result = await cleanupLegacyCommands({ port: queryPort });
      return sendJson(res, 200, { success: true, ...result });
    }
    ...
    if (req.method === "GET" && pathname === "/v1/workbuddy/login/status") {
      const state = url.searchParams.get("state") || "";
      const poller = getActiveLoginState(state);
      return sendJson(res, 200, { success: true, state, poller });
    }

    if (req.method === "POST" && pathname === "/v1/workbuddy/account/test") {
      const body = await readJson(req);
      const uid = body.uid || url.searchParams.get("uid");
      const prompt = body.prompt || "hello";
      const model = body.model || "";
      const result = await testAccount({ uid, prompt, model });
      return sendJson(res, 200, { success: true, ...result });
    }

    if (req.method === "POST" && pathname === "/v1/workbuddy/account/revive") {
      const body = await readJson(req);
      const uid = body.uid || url.searchParams.get("uid");
      const result = await reviveAccount({ uid });
      return sendJson(res, 200, { success: true, ...result });
    }
