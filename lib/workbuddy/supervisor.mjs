...
export function readSessionInfo(sessionFile = DEFAULT_SESSION_FILE) {
  try {
    ...
    const expiresAt = json.expires_at || json.expiresAt || auth.expiresAt || auth.expires_at;
    const user =
      account.nickname ||
      account.name ||
      account.uid ||
...
export async function getStatus({ port = DEFAULT_WORKBUDDY_PORT, sessionFile = "" } = {}) {
  ...
  const pid = readPid(resolvedPidFile, resolvedPort);
  ...
  if (installed) {
    try {
      if (uvPath) {
        const { stdout } = await execFileP(uvPath, ["tool", "list"], { timeout: 3000 });
...
export async function stop({ pidFile = "", port = DEFAULT_WORKBUDDY_PORT } = {}) {
  const resolvedPort = Number(port) || DEFAULT_WORKBUDDY_PORT;
  const resolvedPid = resolvePidFile(resolvedPort, pidFile);
  const pid = readPid(resolvedPid, resolvedPort);
...
export function openInBrowser(targetUrl) {
  try {
    ...
    if (process.platform === "win32") {
      const child = spawn("cmd.exe", ["/c", "start", '""', targetUrl], { detached: true, stdio: "ignore" });
      child.unref();
      return true;
    }
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (tokenData && typeof tokenData === "object" && tokenData.accessToken) {
          ...
          try {
            await stop({ port });
            await start({ endpoint, port, sessionFile });
          } catch {}
