...
export function resolveBinary(name, {
  ...
  const commonDirs = [
    ...
    path.join(home, "Library", "Application Support", "uv", "bin"),
    "/opt/homebrew/bin",
    "/opt/homebrew/sbin",
    "/usr/local/bin",
    "/usr/local/sbin",
  ...
  if (pathValue) {
    ...
    for (const p of envPaths) {
      if (p && !commonDirs.includes(p)) commonDirs.push(p);
    }
  }
...
export async function checkLegacyCommands({ port = DEFAULT_WORKBUDDY_PORT } = {}) {
  const findings = [];
  let uvInfo = { detected: false, version: null, path: null };
  let pipxInfo = { detected: false, version: null, path: null };
  let externalBinInfo = { detected: false, path: null };
  let portConflictInfo = { detected: false, pid: null, port };
  ...
  if (uvPath) {
    try {
      ...
      if (match) {
        uvInfo = { detected: true, version: match[1], path: uvPath };
        findings.push({
          type: "uv",
          label: `uv tool: workbuddy2api (v${match[1]})`,
          detail: `检测到通过 uv tool 安装的旧版驱动 (uv 路径: ${uvPath})`,
          canAutoClean: true,
        });
      }
    } catch {}
  ...
  // 2. Check pipx list
  const pipxPath = resolveBinary("pipx");
  if (pipxPath) {
    try {
      const { stdout } = await execFileP(pipxPath, ["list", "--short"], { timeout: 4000, windowsHide: true });
      const match = stdout.match(/workbuddy2api\s+v?([0-9.]+)/i);
      if (match) {
        pipxInfo = { detected: true, version: match[1], path: pipxPath };
        findings.push({
          type: "pipx",
          label: `pipx: workbuddy2api (v${match[1]})`,
          detail: `检测到通过 pipx 安装的旧版驱动 (pipx 路径: ${pipxPath})`,
          canAutoClean: true,
        });
      }
    } catch {}
  ...
  // 3. Check standalone command in PATH (e.g. workbuddy2api)
  const repoDir = path.resolve(import.meta.dirname, "../..");
  const bundledDir = path.resolve(repoDir, "bin", "workbuddy");
  const extBin = resolveBinary("workbuddy2api");
  if (extBin && !extBin.startsWith(bundledDir)) {
    externalBinInfo = { detected: true, path: extBin };
    findings.push({
      type: "bin",
      label: `系统 PATH 残留命令: workbuddy2api`,
      detail: `检测到系统命令行直接可执行的旧版 workbuddy2api (${extBin})`,
      canAutoClean: false,
    });
  }

  // 4. Check port 7863 external occupancy if our supervisor isn't running
  const supervisorPid = readPid(null, port);
  const isSupervisorAlive = supervisorPid ? isPidAlive(supervisorPid) : false;
  if (!isSupervisorAlive) {
    const activePid = await findPidByPort(port);
    if (activePid && activePid !== process.pid) {
      portConflictInfo = { detected: true, pid: activePid, port };
      findings.push({
        type: "port",
        label: `端口 ${port} 被外部进程占用 (PID: ${activePid})`,
        detail: `有外部未知进程正在占用 WorkBuddy 核心端口 ${port}`,
        canAutoClean: true,
      });
    }
  }
  ...
  return {
    ...
    uv: uvInfo,
    pipx: pipxInfo,
    externalBin: externalBinInfo,
    portConflict: portConflictInfo,
    summary: detected
      ? `检测到 ${findings.length} 处旧版残留环境或指令冲突`
      : "系统环境纯净，未检测到旧版 Python 驱动或冲突指令",
  };
}

export async function cleanupLegacyCommands({ port = DEFAULT_WORKBUDDY_PORT } = {}) {
  const results = [];
  let cleanedCount = 0;
  ...
  if (uvPath) {
    try {
      const { stdout } = await execFileP(uvPath, ["tool", "list"], { timeout: 4000, windowsHide: true });
      if (/workbuddy2api/i.test(stdout)) {
        const { stdout: out, stderr } = await execFileP(
          uvPath,
          ["tool", "uninstall", "workbuddy2api"],
          { timeout: 15000, windowsHide: true }
        );
        results.push({ tool: "uv", ok: true, output: (out + "\n" + stderr).trim() });
        cleanedCount++;
      }
    } catch (err) {
      results.push({ tool: "uv", ok: false, error: err.message });
    }
  }
  ...
  if (pipxPath) {
    try {
      ...
      if (/workbuddy2api/i.test(stdout)) {
        const { stdout: out, stderr } = await execFileP(
          pipxPath,
          ["uninstall", "workbuddy2api"],
          { timeout: 15000, windowsHide: true }
        );
        results.push({ tool: "pipx", ok: true, output: (out + "\n" + stderr).trim() });
        cleanedCount++;
      }
    } catch (err) {
      results.push({ tool: "pipx", ok: false, error: err.message });
    }
  }

  // 3. Port conflict release
  const supervisorPid = readPid(null, port);
  const isSupervisorAlive = supervisorPid ? isPidAlive(supervisorPid) : false;
  if (!isSupervisorAlive) {
    const activePid = await findPidByPort(port);
    if (activePid && activePid !== process.pid) {
      try {
        ...
        results.push({ tool: "port", ok: true, message: `已终止占用端口 ${port} 的外部进程 (PID: ${activePid})` });
        cleanedCount++;
      } catch (err) {
        results.push({ tool: "port", ok: false, error: `无法终止 PID ${activePid}: ${err.message}` });
      }
    }
  }

  const remaining = await checkLegacyCommands({ port });
  return {
    ok: cleanedCount > 0 || !remaining.detected,
    cleanedCount,
    results,
    remaining,
  };
...
export async function checkLegacyUvTool() {
  const res = await checkLegacyCommands();
  return res.uv.detected ? { detected: true, version: res.uv.version, uvPath: res.uv.path } : { detected: false };
}

export async function cleanupLegacyUvTool(port = DEFAULT_WORKBUDDY_PORT) {
  const resolvedPort = typeof port === "object" && port !== null ? (port.port || DEFAULT_WORKBUDDY_PORT) : port;
  return cleanupLegacyCommands({ port: resolvedPort });
}
...
export async function findPidByPort(port = DEFAULT_WORKBUDDY_PORT) {
  ...
  try {
    if (process.platform === "win32") {
      const { stdout } = await execFileP("netstat", ["-ano"], { windowsHide: true });
...
export function syncLegacySessionToAuthDir(sessionFile = DEFAULT_SESSION_FILE, authDir = DEFAULT_AUTHS_DIR) {
  try {
    ...
    const uid = account.uid || json.uid || auth.uid;
    if (!uid) return false;

    fs.mkdirSync(authDir, { recursive: true });
    const targetFile = path.join(authDir, `workbuddy-${uid}.json`);
    if (!fs.existsSync(targetFile)) {
      const isGlobal = auth.realm === "global" || (auth.domain && auth.domain.includes("workbuddy.ai"));
      const authContent = {
        auth: {
          accessToken: token,
          refreshToken: auth.refreshToken || json.refreshToken || "",
          expiresAt: auth.expiresAt || auth.expires_at || json.expiresAt || (Math.floor(Date.now() / 1000) + 86400 * 30),
          domain: auth.domain || (isGlobal ? "www.workbuddy.ai" : "copilot.tencent.com"),
          realm: isGlobal ? "global" : "cn",
        },
        account: account.uid ? account : { uid, nickname: account.nickname || "CodeBuddy 账号" },
        machineId: json.machineId || crypto.randomUUID(),
      };
      fs.writeFileSync(targetFile, JSON.stringify(authContent, null, 2) + "\n", { mode: 0o600 });
      return true;
    }
  } catch {}
  return false;
...
export function listLocalAuthAccounts(authDir = DEFAULT_AUTHS_DIR, sessionFile = DEFAULT_SESSION_FILE) {
  try {
    syncLegacySessionToAuthDir(sessionFile, authDir);
  } catch {}
  const accounts = [];
  try {
...
export async function getStatus({ port = DEFAULT_WORKBUDDY_PORT, sessionFile = "" } = {}) {
  ...
  const legacyCommands = await checkLegacyCommands({ port: resolvedPort });
  const legacyUv = legacyCommands.uv;
...
export async function start({
  ...
  if (autoCleanUv) {
    const legacy = await checkLegacyCommands({ port: resolvedPort });
    if (legacy.detected) {
      await cleanupLegacyCommands({ port: resolvedPort });
    }
  }
...
export async function login({
  ...
  const shouldOpenBrowser = open_browser !== undefined ? Boolean(open_browser) : Boolean(openBrowser);
  const resolvedPort = Number(port) || DEFAULT_WORKBUDDY_PORT;
  const resolvedSession = resolveSessionFile(resolvedPort, sessionFile);
  const { state, authUrl, endpoint: targetEndpoint } = await requestLoginUrl({ endpoint, edition });

  let browserOpened = false;
  if (shouldOpenBrowser && authUrl) {
...
export async function testAccount({
  ...
  const startAt = Date.now();
  if (!uid) {
    return { ok: false, error: "UID 不能为空", latencyMs: 0 };
  }

  if (!fs.existsSync(authDir)) {
    return { ok: false, error: `未找到账号凭据文件 (UID: ${uid})`, latencyMs: 0 };
  }
  ...
  for (const f of files) {
    try {
      ...
      const fileUid = json.account?.uid || json.uid || json.auth?.uid;
      if (fileUid === uid || f === `workbuddy-${uid}.json` || f === `${uid}.json`) {
        targetFile = fullPath;
        authContent = json;
        break;
      }
  ...
  if (!targetFile || !authContent) {
    return { ok: false, error: `未找到账号凭据文件 (UID: ${uid})`, latencyMs: 0 };
  }

  const auth = authContent.auth || {};
  const token = auth.accessToken || auth.token;
  if (!token) {
    return { ok: false, error: "账号凭据缺少有效 AccessToken，请点击「重新授权」", latencyMs: 0 };
  }

  const isGlobal = auth.realm === "global" || (auth.domain && auth.domain.includes("workbuddy.ai"));
  const baseOrigin = isGlobal ? "https://www.workbuddy.ai" : "https://copilot.tencent.com";
  const defaultModel = model || "fast-model";

  let credits = undefined;
  try {
    const billingRes = await fetchImpl(`${baseOrigin}/billing/meter/get-user-resource`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
        Origin: isGlobal ? "https://www.workbuddy.ai" : "https://www.codebuddy.cn",
        Referer: (isGlobal ? "https://www.workbuddy.ai" : "https://www.codebuddy.cn") + "/",
        "User-Agent": "WorkBuddy/5.5.4",
        "X-CodeBuddy-Request": "1",
        Authorization: "Bearer " + token,
        "X-No-Enterprise-Id": "1",
        "X-Domain": isGlobal ? "www.workbuddy.ai" : (auth.domain || "copilot.tencent.com"),
      },
      body: "{}",
    ...
    if (billingRes.ok) {
      ...
      const accounts = billingJson?.data?.Response?.Data?.Accounts;
      if (Array.isArray(accounts) && accounts.length > 0) {
        credits = accounts.reduce((sum, a) => sum + (Number(a.CapacityRemain) || 0), 0);
      }
    }
  } catch {}
  ...
  try {
    const res = await fetchImpl(`${baseOrigin}/v2/chat/completions`, {
      method: "POST",
      headers,
      body: JSON.stringify({
        model: defaultModel,
    ...
    if (!res.ok || (rawText.includes('"code":') && !rawText.startsWith("data:"))) {
      let errObj = null;
      try { errObj = JSON.parse(rawText); } catch {}
      const displayMsg = errObj?.displayMsg?.zh || errObj?.displayMsg?.en;
      const msg = displayMsg || errObj?.msg || `HTTP ${res.status} ${res.statusText}`;
      const code = errObj?.code;
      const isRiskBlocked = code === 11140 || (displayMsg && displayMsg.includes("安全审核")) || (rawText && rawText.includes("request illegal"));
      const advice = isRiskBlocked
        ? "腾讯国际版 (workbuddy.ai) 对该账号触发了推理门禁风控（并非测试提示词违规，剩余积分仍有效）。推荐解决方式：\n1. 优先切换为【国内版】登录（微信/QQ 扫码登录 copilot.tencent.com，支持每日签到且无海外风控）；\n2. 点击【重新授权】刷新 OAuth 凭据；\n3. 在浏览器打开 workbuddy.ai 登录并在网页端发起一次对话激活账号。"
        : null;
      ...
      return {
        ...
        status: res.status,
        code,
        isRiskBlocked: Boolean(isRiskBlocked),
        message: displayMsg ? `${displayMsg} (错误码: ${code})` : (code ? `${msg} (错误码: ${code})` : msg),
        detail: displayMsg || msg,
        advice,
        credits,
        latencyMs,
        model: defaultModel,
        realm: isGlobal ? "global" : "cn",
        account: { uid, nickname: authContent.account?.nickname || authContent.nickname },
      };
    }
    ...
    return {
      ok: true,
      status: 200,
      reply: replyText.trim() || "OK (上游连接正常)",
      credits,
      latencyMs,
      model: defaultModel,
      realm: isGlobal ? "global" : "cn",
    ...
    return {
      ok: false,
      error: err.name === "TimeoutError" ? "请求上游超时（30s）" : err.message,
      credits,
...
export async function reviveAccount({
  ...
  if (!uid) return { ok: false, error: "UID 不能为空" };
  const resolvedStateFile = stateFile || path.join(dataDir, "state.json");
  if (!fs.existsSync(resolvedStateFile)) {
    return { ok: true, message: "状态记录文件无需重置" };
  }
  try {
    const raw = fs.readFileSync(resolvedStateFile, "utf8");
    const json = JSON.parse(raw);
    const accounts = json.accounts || {};
    let found = false;
    for (const [k, val] of Object.entries(accounts)) {
      if (k === uid || k.includes(uid) || uid.includes(k)) {
        val.disabled = false;
        val.reason = "";
        val.disabled_reason = "";
        val.until = "0001-01-01T00:00:00Z";
        val.cool_kind = 0;
        val.consecutive_fails = 0;
        val.session_dead_fails = 0;
        found = true;
      }
    }
    fs.writeFileSync(resolvedStateFile, JSON.stringify(json, null, 2) + "\n", "utf8");

    try {
      const health = await checkHealth(port, 500);
      if (health.ok) {
        await stopImpl({ port });
        await startImpl({ port });
      }
    } catch {}

    return { ok: true, found, message: "账号已解除禁用并重新纳入可用池" };
  } catch (err) {
    return { ok: false, error: err.message };
  }
}
