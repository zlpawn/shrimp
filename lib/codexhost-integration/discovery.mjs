...
export async function discoverCodexhostExecutable({
  ...
  const platformPackageName = `@codexhost/cli-${packageSuffix}`;
  const launcherName = `codexhost${platform === "win32" ? ".exe" : ""}`;
  const launcherCandidates = [
    path.join(globalRoot, "@codexhost", `cli-${packageSuffix}`, "bin", launcherName),
    path.join(globalRoot, "@codexhost", "cli", "node_modules", platformPackageName, "bin", launcherName),
  ];
  const launcherPath = launcherCandidates.find(fileExists) || "";
  if (!fileExists(entrypointPath) || !launcherPath) return null;
  let version = "";
  try {
