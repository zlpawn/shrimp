...
export async function routeCodexhostRequest(req, res, reqPath, { service }) {
  try {
    ...
    if (pathOnly === "/v1/cli-tools/codexhost/install" && method === "POST") {
      return sendJson(res, 200, await service.install(await readJson(req)));
    }
    if (pathOnly === "/v1/cli-tools/codexhost/update" && method === "POST") {
      return sendJson(res, 200, await service.update(await readJson(req)));
    }
    if (pathOnly === "/v1/cli-tools/codexhost/uninstall" && method === "POST") {
      return sendJson(res, 200, await service.uninstall(await readJson(req)));
    }
