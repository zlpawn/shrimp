...
async function defaultNpmRunner(args, {
  ...
  try {
    return await execFileImpl(npmCmd, args, {
      encoding: "utf8",
      timeout: timeoutMs,
      windowsHide: true,
    ...
    if (error?.code === "ENOENT") {
      throw new CodexhostIntegrationError("executable_not_found", "系统中未检测到 npm，请先安装 Node.js 与 npm。");
    }
    const message = error.stderr ? String(error.stderr).trim() : error.message;
    throw new CodexhostIntegrationError("package_manager_error", `执行 npm ${args.join(" ")} 失败: ${message}`);
  }
}
...
async function defaultGatewayProbe(url, port = null) {
  try {
    const response = await fetch(url, { signal: AbortSignal.timeout(1500) });
    if (!response.ok) return null;
    return { ...(await response.json()), ...(port === null ? {} : { port }) };
  } catch {
    return null;
  }
}
...
export function createCodexhostService({
  ...
  probeGateway = () => defaultGatewayProbe(`http://127.0.0.1:${gatewayPort}/health`, gatewayPort),
  probeConfigGateway = defaultGatewayProbe,
  ...
  execNpm = (args) => defaultNpmRunner(args, { platform }),
  now = () => new Date(),
} = {}) {
  ...
  async function snapshot(operation = "") {
    ...
    const config = await inspectCodexConfig(configSource?.text || "", {
      configPath: configSource?.path || "",
      gatewayPort,
      probeGateway: probeConfigGateway,
    });
  ...
  return {
    ...
    async start() {
      ...
      if (!state.config.healthy) {
        throw new CodexhostIntegrationError("codex_config_invalid", "Codex 模型配置指向的 Shrimp 网关不可用；为保护现有配置，codexhost 未启动。", { issues: state.config.issues });
      }
    ...
    async install() {
      ...
      await execNpm(["install", "-g", "@codexhost/cli"]);
      return this.getStatus();
    },
    ...
    async update() {
      ...
      await execNpm(["install", "-g", "@codexhost/cli@latest"]);
      return this.getStatus();
    },
    ...
    async uninstall() {
      ...
      await execNpm(["uninstall", "-g", "@codexhost/cli"]);
      return this.getStatus();
    },
  };
}
