...
test("npm package ships the in-repo wendao CLI and managed skill", async () => {
  ...
  assert.ok(fs.existsSync(path.join(projectRoot, "lib", "skills", "leo-xiecheng-wendao", "scripts", "cli", "index.mjs")));
...
test("npm package ships the leo database hub CLI and skill", async () => {
  const pkg = JSON.parse(await readFile(path.join(projectRoot, "package.json"), "utf8"));
  assert.ok(pkg.files.includes("lib"));
  assert.ok(fs.existsSync(path.join(projectRoot, "lib", "skills", "leo-database-hub", "scripts", "cli", "index.mjs")));
  assert.ok(fs.existsSync(path.join(projectRoot, "lib", "skills", "leo-database-hub", "scripts", "mcp", "index.mjs")));
...
test("npm package ships the leo tdx CLI and managed skill", async () => {
  const pkg = JSON.parse(await readFile(path.join(projectRoot, "package.json"), "utf8"));
  assert.ok(pkg.files.includes("lib"));
  assert.ok(fs.existsSync(path.join(projectRoot, "lib", "skills", "leo-tdx-stock", "scripts", "cli", "index.mjs")));
...
test("syntax check covers every shipped in-repo CLI entrypoint", async () => {
  const pkg = JSON.parse(await readFile(path.join(projectRoot, "package.json"), "utf8"));
  assert.match(pkg.scripts.check, /lib\/skills\/leo-xiecheng-wendao\/scripts\/cli\/index\.mjs/u);
  assert.match(pkg.scripts.check, /lib\/skills\/leo-database-hub\/scripts\/cli\/index\.mjs/u);
});

test("syntax check covers the leo tdx CLI entrypoint", async () => {
  const pkg = JSON.parse(await readFile(path.join(projectRoot, "package.json"), "utf8"));
  assert.match(pkg.scripts.check, /lib\/skills\/leo-tdx-stock\/scripts\/cli\/index\.mjs/u);
});
