...
test("remote session panels and dialogs use spacious grouped layouts", () => {
  ...
  assert.equal((catalog.match(/class="endpoints-grid rs-catalog-grid"/g) || []).length, 2);
  assert.match(catalog, /Antigravity 远程编码[\s\S]*Antigravity 官方远程控制/);
  assert.match(catalog, /IM 会话投递/);
  assert.match(css, /.rs-catalog-section\s*\{[^}]*display:\s*grid/s);
