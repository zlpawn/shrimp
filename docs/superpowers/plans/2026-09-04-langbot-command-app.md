...
- [x] Write failing tests for LangBot registry defaults, normalization, invalid persisted JSON, and store round-trip.
- [x] Implement registry fields, normalization/validation, and SQLite daemon settings persistence.
- [x] Run `node --test tests/unit/command-apps.test.mjs`.
...
- [x] Write failing tests for URL resolution, health checks, startup environment/cwd, managed PID tracking, stop, and process-exit reporting.
- [x] Implement the adapter using detached spawn, HTTP probe, process-liveness checks, and process-tree termination.
- [x] Run focused tests.
...
- [x] Write failing service/route tests for path discovery, launch settings, config update, install, and update safety.
- [x] Integrate the LangBot adapter and add install/update routes without changing Hindsight branches.
- [x] Run focused tests.
...
- [x] Write failing tests showing install uses `uv tool install langbot`, update stops first and uses `uv tool upgrade langbot`, failures retain prior path/data, and version discovery succeeds after upgrade.
- [x] Implement with promisified `execFile`, sanitized env, explicit timeouts, non-destructive error handling, and post-install discovery.
- [x] Run focused tests.
...
- [x] Extend status/action types and state.
- [x] Render a LangBot-specific card and LLM guidance.
- [x] Add browser handlers and busy-state handling.
...
- [x] Run `npm run build:panel`.
- [x] Run `npm run check`.
- [x] Inspect git diff and ensure no unrelated workspace files are modified.
