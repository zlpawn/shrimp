...
此外，用户在笔记本与台式机双机协作时，需要跨机器的会话查看、终端接管与双向同步能力。引入 Cloudflare Tunnel 作为底层跨公网加密通道，可以实现无公网 IP 限制的对等互联（Peer-to-Peer Duplex）。
...
4. **多端对等互联（Peer-to-Peer Duplex）**：
   ...
   - 实现全双工终端接管（通过 WebSocket 穿透 Cloudflare Tunnel）。
   - 实现会话增量同步协议（双向对比清单、Pull/Push 会话 JSON 到 `~/.shrimp/hub/sessions/`）。
...
2. 不要求服务端必须拥有已备案的国内域名（Quick 模式完全零门槛开箱即用）。
...
#### 同步协议接口
在网关新增轻量 REST 端点，供两台机器的双向 Session Sync 调用：
1. `GET /v1/session-sync/manifest`
   - 返回本地 `~/.shrimp/hub/sessions/` 下所有会话文件的简要列表：`[{ sessionId, filename, updatedAt, workspace, size }]`。
2. `GET /v1/session-sync/file/:sessionId`
   - 下载指定会话的完整 JSON 内容。
3. `POST /v1/session-sync/push`
   - 将本地修改/新建的会话推送到对端 Hub。
...
1. **单元测试**：
   ...
   - `tests/unit/session-sync-peer-protocol.test.mjs`：测试 manifest 接口与文件 pull/push 逻辑。
2. **集成测试**：
