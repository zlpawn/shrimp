...
window.switchTab = function(tabId) {
    ...
    if (['dream-skin', 'nat-traversal', 'command-apps', 'mcp-management'].includes(tabId)) {
        try {
            runTabEnter(tabId);
        } catch (error) {
...
window.addEventListener('load', () => {
    ...
    const parts = hash.split('/');
    const tabId = parts[0];
    const knownTabs = ['code','desktop','codex','deeptutor','analytics','proxy','sync','skills','install-history','tools','extensions','cli','cli-install-history','cli-sources','dream-skin','nat-traversal','command-apps','mcp-management'];
